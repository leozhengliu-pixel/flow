...
func TestMCPMissingReferencesAndInvalidPatchesDoNotWrite(t *testing.T) {
	...
	for _, scenario := range []struct {
		...
		{"get_agent_skill", map[string]any{"id": "missing"}}, {"get_issue_status", map[string]any{"team": f.data.Teams[0].ID}},
		{"get_issue_status", map[string]any{"team": f.data.Teams[0].ID, "name": "missing"}},
