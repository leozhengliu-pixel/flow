...
func TestEnterpriseIdentityGitAndDeliveryPersistence(t *testing.T) {
	...
	provider := requestJSON[domain.IdentityProvider](t, handler, http.MethodPost, "/api/identity-providers?workspace=cleantrack", map[string]any{
	...
	requestJSON[domain.IdentityProvider](t, handler, http.MethodPatch, "/api/identity-providers/"+provider.ID+"?workspace=cleantrack", map[string]any{"enforced": true}, http.StatusOK)

	team := bootstrap.Teams[0]
	state := bootstrap.States[0]
	automation := requestJSON[domain.GitAutomationState](t, handler, http.MethodPost, "/api/git-automations?workspace=cleantrack", map[string]any{"teamId": team.ID, "repository": "acme/api", "event": "pull_request.merged", "workflowStateId": state.ID, "syncComments": true, "enabled": true}, http.StatusOK)
	if automation.ID == "" || !automation.SyncComments {
		t.Fatalf("automation not persisted: %#v", automation)
	}
	branch := requestJSON[domain.TargetBranch](t, handler, http.MethodPost, "/api/target-branches?workspace=cleantrack", map[string]any{"teamId": team.ID, "repository": "acme/api", "branch": "main", "default": true}, http.StatusOK)
	if branch.ID == "" || !branch.Default {
		t.Fatalf("branch not persisted: %#v", branch)
	}

	connection := requestJSON[domain.IntegrationConnection](t, handler, http.MethodPut, "/api/integrations/slack?workspace=cleantrack", map[string]any{"name": "Slack", "config": map[string]string{"deliveryURL": "https://hooks.example.com/flow"}, "scopes": []string{"events:write"}, "channels": []string{"engineering"}, "linkbackEnabled": true}, http.StatusOK)
	delivery := requestJSON[domain.IntegrationDelivery](t, handler, http.MethodPost, "/api/integration-deliveries?workspace=cleantrack", map[string]any{"connectionId": connection.ID, "eventType": "issue.updated", "resourceId": "issue_1", "channel": "engineering", "payload": map[string]any{"id": "issue_1"}}, http.StatusAccepted)
	if delivery.Status != "pending" || delivery.Channel != "engineering" {
		t.Fatalf("delivery not queued: %#v", delivery)
	}

	updated, _ := repository.BootstrapFor("cleantrack")
...
func TestIdentityProviderValidationAndPublicDiscovery(t *testing.T) {
	...
	requestJSON[map[string]any](t, handler, http.MethodPost, "/api/identity-providers?workspace=cleantrack", map[string]any{"type": "oidc", "name": "Bad", "issuer": "http://insecure.example", "clientId": "x"}, http.StatusUnprocessableEntity)
