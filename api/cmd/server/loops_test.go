...
func TestLoopLifecycle(t *testing.T) {
	...
	created := requestJSON[domain.Loop](t, handler, http.MethodPost, "/api/loops", map[string]any{
		"name": "Daily issue triage", "triggerType": "issue", "triggerConfig": map[string]any{"action": "created"}, "instructions": "Assign new issues to the right team.", "allowChangesOutsideTrigger": true,
	}, http.StatusCreated)
	if created.ID == "" || created.Name != "Daily issue triage" || created.TriggerType != "issue" || !created.Enabled {
