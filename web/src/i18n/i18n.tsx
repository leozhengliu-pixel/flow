...
export function translateToChinese(source: string): string {
  ...
  const patterns: Array<[RegExp, (...groups: string[]) => string]> = [
    ...
    [/^(\d+) projects? · (\d+) issues?$/, (projects, issues) => `${projects} 个项目 · ${issues} 个问题`],
    ...
    [/^Import (\d+) issues?$/, count => `导入 ${count} 个问题`],
    ...
    [/^Activity, (\d+) issues?$/, count => `动态，${count} 个问题`],
    [/^Select issue(?: (.+))?$/, detail => `选择问题${detail ? `：${translateEmbedded(detail)}` : ''}`],
