...
export interface Loop { id: UUID; name: string; icon?: string; level: 'workspace'|'team'; triggerType: 'schedule'|'issue'|'project'|'initiative'|'cycle'; triggerConfig?: Record<string, unknown>; instructions: string; connectorIds: UUID[]; teamAccess: 'allPublic'|'selected'; allowChangesOutsideTrigger: boolean; allowExternalSync: boolean; enabled: boolean; creator: User; lastRunAt?: string; nextRunAt?: string; createdAt: string; updatedAt: string }
