import type { CodeReview, Cycle, Initiative, Issue, Project, SavedView } from '@/types/flow'
...
const MY_ISSUES_VIEWS = new Set<MyIssuesRouteView>(['assigned', 'created', 'subscribed', 'activity'])
const PROJECT_TABS = new Set<ProjectRouteTab>(['overview', 'activity', 'issues'])
...
export function parseAppRoute(pathname: string): AppRoute {
  ...
  if (segments.length === 1 && segments[0] === 'join') return { kind: 'workspace-onboarding' }
  const [workspaceSlug, section, third, fourth, fifth, sixth] = segments
  if (!section) return { kind: 'workspace-root', workspaceSlug }
  if (section === 'inbox' && segments.length === 2) return { kind: 'inbox', workspaceSlug }
  ...
  if (section === 'reviews' && third === 'created' && segments.length === 3) return { kind: 'reviews', workspaceSlug, view: 'created' }
  if (section === 'review' && third && segments.length === 3) return { kind: 'review', workspaceSlug, reviewSlug: third, tab: 'overview' }
  if (section === 'review' && third && (fourth === 'review' || fourth === 'changes') && segments.length === 4) return { kind: 'review', workspaceSlug, reviewSlug: third, tab: fourth }
  if (section === 'issues' && !third) return { kind: 'workspace-issues', workspaceSlug, view: 'all' }
  if (section === 'issues' && TEAM_ISSUES_VIEWS.has(third as TeamIssuesRouteView) && segments.length === 3) return { kind: 'workspace-issues', workspaceSlug, view: third as TeamIssuesRouteView }
  ...
  if (section === 'loops' && !third && segments.length === 2) return { kind: 'loops', workspaceSlug }
  if (section === 'loops' && third === 'new' && segments.length === 3) return { kind: 'loop-editor', workspaceSlug }
  if (section === 'loops' && third && segments.length === 3) return { kind: 'loop-editor', workspaceSlug, loopId: third }
  ...
  if (section === 'settings' && third === 'templates' && fourth === 'project' && fifth === 'new' && !sixth && segments.length === 5) return { kind: 'settings', workspaceSlug, page: 'project-templates', projectTemplateMode: 'new' }
  if (section === 'settings' && third === 'templates' && fourth === 'project' && fifth && sixth === 'edit' && segments.length === 6) return { kind: 'settings', workspaceSlug, page: 'project-templates', projectTemplateMode: 'edit', projectTemplateId: fifth }
  ...
  if (section === 'settings' && third === 'releases' && fourth === 'pipelines' && fifth && segments.length === 5) return { kind: 'settings', workspaceSlug, page: 'releases', releasePipelineMode: 'edit', releasePipelineSlug: fifth }
  if (section === 'settings' && third === 'integrations' && (fourth === 'github' || fourth === 'gitlab') && segments.length === 4) return { kind: 'settings', workspaceSlug, page: 'integrations', integrationProvider: fourth }
  ...
  if (section === 'initiatives' && (third === 'active' || third === 'planned') && segments.length === 3) return { kind: 'initiatives', workspaceSlug, view: third }
	if (section === 'initiative' && third && !fourth) return { kind: 'initiative', workspaceSlug, initiativeSlugId: third, tab: 'overview' }
	if (section === 'initiative' && third && fourth === 'updates') return { kind: 'initiative', workspaceSlug, initiativeSlugId: third, tab: 'activity' }
	if (section === 'initiative' && third && fourth === 'view' && fifth === 'new' && segments.length === 5) return { kind: 'initiative', workspaceSlug, initiativeSlugId: third, tab: 'new' }
  ...
  if (section === 'project' && third && fourth === 'updates' && segments.length === 4) return { kind: 'project', workspaceSlug, projectSlugId: third, tab: 'activity' }
  if (section === 'project' && third && fourth === 'view' && fifth === 'new' && segments.length === 5) return { kind: 'project', workspaceSlug, projectSlugId: third, tab: 'new' }
  if (section === 'project' && third && PROJECT_TABS.has(fourth as ProjectRouteTab) && segments.length === 4) return { kind: 'project', workspaceSlug, projectSlugId: third, tab: fourth as ProjectRouteTab }
  if (section === 'issue' && third && segments.length <= 4) return { kind: 'issue', workspaceSlug, identifier: third.toUpperCase(), titleSlug: fourth }
  return { kind: 'not-found', workspaceSlug }
}

export function workspaceRootPath(workspaceSlug: string) { return `/${encode(workspaceSlug)}` }
export function workspaceOnboardingPath() { return '/join' }
...
export function loopsPath(workspaceSlug: string) { return `${workspaceRootPath(workspaceSlug)}/loops` }
export function newLoopPath(workspaceSlug: string) { return `${loopsPath(workspaceSlug)}/new` }
export function loopPath(workspaceSlug: string, loopId: string) { return `${loopsPath(workspaceSlug)}/${encode(loopId)}` }
...
export function newIssueTemplatePath(workspaceSlug:string,form=false){return `${workspaceRootPath(workspaceSlug)}/settings/templates/issue/new${form?'/form':''}`}
export function issueTemplateEditPath(workspaceSlug:string,templateId:string){return `${workspaceRootPath(workspaceSlug)}/settings/templates/issue/${encode(templateId)}/edit`}
export function newProjectTemplatePath(workspaceSlug:string){return `${workspaceRootPath(workspaceSlug)}/settings/templates/project/new`}
export function projectTemplateEditPath(workspaceSlug:string,templateId:string){return `${workspaceRootPath(workspaceSlug)}/settings/templates/project/${encode(templateId)}/edit`}
export function integrationSettingsPath(workspaceSlug:string,provider:'github'|'gitlab'){return `${workspaceRootPath(workspaceSlug)}/settings/integrations/${provider}`}
...
export function savedViewPathId(view: Pick<SavedView, 'id' | 'slugId'>) { return view.slugId || view.id }
...
export function routeBelongsToWorkspace(route: AppRoute, workspaceSlug: string) { return route.kind === 'root' || route.kind === 'workspace-onboarding' || route.workspaceSlug === workspaceSlug }
