...
export function createReleasePipeline(input: {name:string;teamIds?:string[];type?:ReleasePipeline['type'];production?:boolean;stages?:string[]}):Promise<ReleasePipeline>{return request('/api/release-pipelines',jsonRequest('POST',input))}
export function updateReleasePipeline(id:string,input:Partial<Pick<ReleasePipeline,'name'|'teamIds'|'type'|'production'|'stages'>> & {archived?:boolean}):Promise<ReleasePipeline>{return request(`/api/release-pipelines/${id}`,jsonRequest('PATCH',input))}
