...
function ValueSummary({ values }: { values: ReturnType<typeof filterValues> }) {
  ...
  return <><span className={styles.valueStack}>{values.slice(0, 3).map((value, index) => <i key={value.value} style={{ backgroundColor: value.color ?? ['var(--theme-text-secondary)', 'var(--theme-text-tertiary)', 'var(--status-neutral)'][index], zIndex: 3 - index }}/>)}</span><span data-i18n-ignore>{values.map(value => value.valueLabel).join(', ')}</span></>
}
