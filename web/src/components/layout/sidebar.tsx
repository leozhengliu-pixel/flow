...
import { addFavorite, addSubscription, removeFavorite, removeSubscription, setTeamMembership } from '@/lib/api'
...
import { agentPath, asksPath, currentCyclePath, customersPath, documentsPath, draftsPath, inboxPath, initiativesPath, membersPath, myIssuesPath, projectsPath, pulsePath, releasePipelinesPath, reviewsPath, settingsPath, teamArchivePath, teamCyclesPath, teamHomePath, teamInitiativesPath, teamIssuesPath, teamProjectsPath, teamsPath, teamViewsPath, upcomingCyclePath, workspaceViewsPath, loopsPath } from '@/lib/app-routes'
import type { AccountBootstrap, BootstrapData, Team, Workspace } from '@/types/flow'

import './sidebar.css'

export type PageId = 'inbox' | 'search' | 'pulse' | 'reviews' | 'my-issues' | 'workspace-issues' | 'team-issues' | 'team-archive' | 'cycles' | 'cycle-detail' | 'projects' | 'views' | 'project-detail' | 'issue-detail' | 'initiatives' | 'initiative-detail' | 'members' | 'customers' | 'teams' | 'new-team' | 'drafts' | 'agent' | 'loops' | 'releases' | 'asks' | 'documents' | 'analytics' | 'document-detail' | 'customer-detail'

type SidebarEntry = 'inbox' | 'reviews' | 'myIssues' | 'pulse' | 'drafts' | 'agent' | 'initiatives' | 'projects' | 'documents' | 'views' | 'members' | 'customers' | 'teams' | 'releases' | 'loops'
...
const defaultWorkspaceOrder: SidebarEntry[] = ['members', 'initiatives', 'projects', 'documents', 'teams', 'views', 'releases', 'loops', 'customers']
...
const defaultPreferences: SidebarPreferences = {
  ...
  initiatives: 'always', projects: 'always', documents: 'always', views: 'always', members: 'always', customers: 'never', teams: 'always', releases: 'always', loops: 'always',
}
...
export function Sidebar({ account, data, page, open = false, onOpenChange, onSearch, onCreate, onOpenSettings, onSwitchWorkspace, onCreateWorkspace, onLogout }: {
  ...
  const activeWorkspaceEntry: SidebarEntry | undefined = page === 'members' ? 'members' : page === 'customers' || page === 'customer-detail' ? 'customers' : page === 'teams' || page === 'new-team' ? 'teams' : page === 'documents' || page === 'document-detail' ? 'documents' : page==='releases'?'releases':page==='loops'?'loops':undefined
  ...
  const personalNavigation: Record<SidebarEntry, ReactNode> = {
    ...
    reviews: <Nav badge={reviewCount} active={page==='reviews'} icon={<ReviewsIcon/>} label="Reviews" to={reviewsPath(workspaceSlug)} onClick={close}/>,
    myIssues: <Nav active={page === 'issue-detail'} icon={<MyIssuesIcon/>} label="My issues" to={myIssuesPath(workspaceSlug, 'activity')} onClick={close}/>,
    ...
    initiatives: null, projects: null, documents: null, views: null, members: null, customers: null, teams: null, releases:null, loops:null,
  }
  const workspaceNavigation: Record<SidebarEntry, ReactNode> = {
    ...
    documents: <Nav active={page === 'documents' || page === 'document-detail'} icon={<BookOpen/>} label="Documents" to={documentsPath(workspaceSlug)} onClick={close}/>,
    ...
    <aside className={`sidebar ${open ? 'open' : ''}`} aria-label="Workspace navigation">
      ...
      <nav ref={navRef} className="sidebar-nav">
        ...
        <Section label="Workspace" storageKey="workspace">
          ...
          <MoreMenu entries={hiddenWorkspaceEntries} onCustomize={() => setCustomizeOpen(true)} workspaceSlug={workspaceSlug} asks={featureEnabled('asks')}/>
        </Section>
        ...
        <Section label="Your teams" storageKey="teams" action={<NavLink className="section-action" aria-label="Join a team" title="Join a team" to={teamsPath(workspaceSlug)} onClick={close}><Plus/></NavLink>}>
          <div className="sidebar-team-list">
            ...
            current={data.cycles.some(cycle=>cycle.teamId===team.id&&cycle.status==='current')}
            upcoming={data.cycles.some(cycle=>cycle.teamId===team.id&&cycle.status==='upcoming')}
            initiativesEnabled={data.viewerRole!=='guest'&&featureEnabled('initiatives')}
            team={team}
            favorite={data.favorites.some(item=>item.userId===data.viewer.id&&item.resourceType==='team'&&item.resourceId===team.id)}
            subscribed={data.subscriptions.some(item=>item.userId===data.viewer.id&&item.resourceType==='team'&&item.resourceId===team.id)}
            viewerId={data.viewer.id}
            canLeave={data.teams.length>1}
...
function Section({ label, children, action, storageKey }: { label: string; children: ReactNode; action?: ReactNode; storageKey: string }) {
  ...
  return <section className={`nav-section sidebar-section-${storageKey}`} data-expanded={expanded}>
    <div className="section-heading">
      <button className="section-label" type="button" aria-expanded={expanded} onClick={() => setExpanded(value => !value)}><span>{label}</span><SectionDisclosureIcon expanded={expanded}/></button>
      {action}
    </div>
...
function TeamNavigation({ team, workspaceSlug, page, cyclesEnabled, current, upcoming, initiativesEnabled, favorite:initialFavorite, subscribed:initialSubscribed, viewerId, canLeave, onNavigate }: { team: Team; workspaceSlug: string; page: PageId | 'not-found'; cyclesEnabled: boolean;current:boolean;upcoming:boolean;initiativesEnabled:boolean;favorite:boolean;subscribed:boolean;viewerId:string;canLeave:boolean;onNavigate: () => void }) {
  const [expanded, setExpanded] = useState(() => readExpandedSection(`team.${team.id}`))
  const[favorite,setFavorite]=useState(initialFavorite),[subscribed,setSubscribed]=useState(initialSubscribed),[query,setQuery]=useState('')
  ...
  return <div className="sidebar-team" data-expanded={expanded}>
    <div className="team-heading">
      ...
      <DropdownMenu.Root onOpenChange={open=>{if(!open)setQuery('')}}><DropdownMenu.Trigger asChild><button className="team-menu-trigger" type="button" aria-label="Team menu"><TeamMenuIcon/></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content className="sidebar-popover sidebar-team-menu" side="right" align="start" sideOffset={4}>
        <div className="sidebar-team-menu-search"><Search/><input aria-label="Filter…" autoFocus placeholder="Filter…" value={query} onChange={event=>setQuery(event.target.value)}/></div>
        {menuMatches(query,'Favorite')&&<DropdownMenu.Item onSelect={()=>void(favorite?removeFavorite('team',team.id):addFavorite('team',team.id)).then(()=>setFavorite(value=>!value))}><Star fill={favorite?'currentColor':'none'}/><span>{favorite?'Unfavorite':'Favorite'}</span></DropdownMenu.Item>}
        <DropdownMenu.Separator/>
        {menuMatches(query,'Team settings')&&<DropdownMenu.Item asChild><NavLink to={settingsPath(workspaceSlug,'team',team.key)}><Settings/><span>Team settings</span></NavLink></DropdownMenu.Item>}
        {menuMatches(query,'Copy URL')&&<DropdownMenu.Item onSelect={()=>void navigator.clipboard.writeText(`${location.origin}${teamHomePath(workspaceSlug,team.key)}`)}><Copy/><span>Copy URL</span></DropdownMenu.Item>}
        {menuMatches(query,'Open archive')&&<DropdownMenu.Item asChild><NavLink to={teamArchivePath(workspaceSlug,team.key)}><Archive/><span>Open archive</span></NavLink></DropdownMenu.Item>}
        <DropdownMenu.Separator/>
        {menuMatches(query,'Subscribe')&&<DropdownMenu.Item onSelect={()=>void(subscribed?removeSubscription('team',team.id):addSubscription('team',team.id,['updates'])).then(()=>setSubscribed(value=>!value))}><Bell/><span>{subscribed?'Unsubscribe':'Subscribe'}</span></DropdownMenu.Item>}
        {menuMatches(query,'Configure Slack notifications')&&<DropdownMenu.Item asChild><NavLink to={settingsPath(workspaceSlug,'team',team.key,'notifications')}><SidebarSlackIcon/><span>Configure Slack notifications…</span></NavLink></DropdownMenu.Item>}
        <DropdownMenu.Separator/>
        <DropdownMenu.Item disabled={!canLeave} title={!canLeave?'You should be a member of at least one active team':undefined} onSelect={()=>canLeave&&void setTeamMembership(workspaceSlug,team.id,viewerId,false)}><LogOut/><span>Leave team…</span></DropdownMenu.Item>
      </DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
    </div>
    {expanded && <div className="team-links">
      <Nav active={onOverview} icon={<HomeIcon/>} label="Home" to={overviewPath} onClick={onNavigate}/>
      ...
      {cyclesEnabled&&(current||upcoming)&&<div className="team-cycle-links">{current&&<SubNav label="Current" to={currentCyclePath(workspaceSlug,team.key)} onClick={onNavigate}/>} {upcoming&&<SubNav label="Upcoming" to={upcomingCyclePath(workspaceSlug,team.key)} onClick={onNavigate}/>}<i aria-hidden="true"/></div>}
      {initiativesEnabled&&<Nav icon={<InitiativeIcon/>} label="Initiatives" to={teamInitiativesPath(workspaceSlug,team.key)} onClick={onNavigate}/>}
...
function MoreMenu({ entries, onCustomize, workspaceSlug, asks }: { entries: SidebarEntry[]; onCustomize: () => void; workspaceSlug: string; asks: boolean }) {
  const items: Partial<Record<SidebarEntry, { label: string; icon: ReactElement; onSelect?: () => void; to?: string }>> = {
    initiatives: { label: 'Initiatives', icon: <InitiativeIcon/>, to: initiativesPath(workspaceSlug) },
    ...
    documents:{label:'Documents',icon:<BookOpen/>,to:documentsPath(workspaceSlug)},
  }
...
function ReviewsIcon(){return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M12.5 10C13.8807 10 15 11.1193 15 12.5C15 13.8807 13.8807 15 12.5 15C11.1193 15 10 13.8807 10 12.5C10 11.1193 11.1193 10 12.5 10ZM12.5 11.5C11.9477 11.5 11.5 11.9477 11.5 12.5C11.5 13.0523 11.9477 13.5 12.5 13.5C13.0523 13.5 13.5 13.0523 13.5 12.5C13.5 11.9477 13.0523 11.5 12.5 11.5Z"/><path d="M3.5 4.5C3.91414 4.50009 4.25 4.83584 4.25 5.25V14.249C4.24982 14.663 3.91403 14.9989 3.5 14.999C3.0859 14.999 2.75018 14.6631 2.75 14.249V5.25C2.75 4.83579 3.08579 4.5 3.5 4.5Z"/><path d="M10 2.75C11.7949 2.75 13.25 4.20507 13.25 6V10.75C13.25 11.1642 12.9142 11.5 12.5 11.5C12.0858 11.5 11.75 11.1642 11.75 10.75V6C11.75 5.0335 10.9665 4.25 10 4.25H8C7.58579 4.25 7.25 3.91421 7.25 3.5C7.25 3.08579 7.58579 2.75 8 2.75H10Z"/></svg>}
function MyIssuesIcon(){return <FlowIcon name="MyIssues"/>}
function PulseIcon() { return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M11.3354 1.08228C11.7059 1.27438 11.8561 1.74156 11.6708 2.12576L9.5593 6.50508C9.39923 6.83707 9.64113 7.22224 10.0097 7.22224H14C14.4142 7.22224 14.75 7.57046 14.75 8C14.75 8.42954 14.4142 8.77776 14 8.77776C10.4465 8.77776 7.32888 11.131 5.67083 14.5699C5.48559 14.9541 5.03507 15.1098 4.66459 14.9177C4.29411 14.7256 4.14394 14.2584 4.32918 13.8742L6.44081 9.49467C6.60085 9.16275 6.35907 8.77764 5.99059 8.77759H1.75C1.33579 8.77759 1 8.42937 1 7.99982C1 7.57028 1.33579 7.22206 1.75 7.22206C5.55362 7.22206 8.67116 4.86885 10.3292 1.43003C10.5145 1.04588 10.965 0.890196 11.3354 1.08228Z"/></svg> }
function AgentIcon() { return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M4.07132 3.8283C4.04394 3.81721 4.01406 3.81379 3.98488 3.8184C3.95566 3.82301 3.92826 3.83551 3.90561 3.85453C3.88297 3.87356 3.86594 3.8984 3.85636 3.92639C3.84678 3.95437 3.84501 3.98443 3.85124 4.01335L5.80802 13.1405C5.81898 13.1915 5.83884 13.2155 5.85542 13.2298C5.87605 13.2476 5.9078 13.2631 5.94754 13.268C5.98728 13.2729 6.0217 13.2654 6.04578 13.2532C6.06507 13.2434 6.08993 13.2252 6.11273 13.1784L7.83779 9.64746C8.05513 9.20258 8.45077 8.87059 8.92663 8.73378L12.7035 7.64791C12.7535 7.63353 12.776 7.61215 12.789 7.59475C12.8052 7.57307 12.8186 7.54044 12.8207 7.50049C12.8228 7.46054 12.813 7.42669 12.7992 7.40342C12.788 7.38476 12.7681 7.36116 12.7199 7.34158L4.07132 3.8283ZM3.75083 2.33677C4.04945 2.2896 4.35527 2.32474 4.63541 2.43841L13.2843 5.95183C14.747 6.54596 14.6351 8.65343 13.1179 9.08953L9.34109 10.1754C9.27311 10.1949 9.21659 10.2424 9.18554 10.3059L7.46077 13.8363C6.76755 15.2562 4.67275 14.9979 4.34147 13.4555L2.38492 4.3294C2.32134 4.03401 2.33935 3.72642 2.43722 3.44054C2.53514 3.15452 2.70919 2.90061 2.94065 2.70612C3.17211 2.51164 3.45221 2.38394 3.75083 2.33677Z"/></svg> }
...
function HomeIcon(){return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.75 14.25V6.48a.75.75 0 0 1 .242-.553l4.5-4.136a.75.75 0 0 1 1.016 0l4.5 4.136a.75.75 0 0 1 .242.553v7.77h-3.5v-3.267a1.75 1.75 0 0 0-3.5 0v3.267h-3.5Z" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/></svg>}
function CycleIcon() { return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 1C8.41421 1 8.75 1.33579 8.75 1.75C8.75 2.16421 8.41421 2.5 8 2.5C4.96243 2.5 2.5 4.96243 2.5 8C2.5 11.0376 4.96243 13.5 8 13.5C11.0376 13.5 13.5 11.0376 13.5 8C13.5 6.23619 12.6701 4.66687 11.377 3.65918C11.0503 3.40457 10.9925 2.93313 11.2471 2.60645C11.5017 2.27972 11.9731 2.22098 12.2998 2.47559C13.9418 3.75527 15 5.75433 15 8C15 11.866 11.866 15 8 15C4.13401 15 1 11.866 1 8C1 4.13401 4.13401 1 8 1Z"/><path d="M6.85547 6.1123C7.07646 5.97571 7.35255 5.96305 7.58496 6.0791L10.085 7.3291L10.1758 7.38281C10.3769 7.52151 10.5 7.75131 10.5 8C10.5 8.24869 10.3769 8.47849 10.1758 8.61719L10.085 8.6709L7.58496 9.9209C7.35255 10.0369 7.07646 10.0243 6.85547 9.8877C6.63458 9.75099 6.5 9.50981 6.5 9.25V6.75C6.5 6.49019 6.63458 6.24901 6.85547 6.1123Z"/></svg> }
function InitiativeIcon(){return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M7.4145 8.3381C7.68162 7.8873 8.31838 7.8873 8.5855 8.3381L11.896 13.925C12.2589 14.5374 11.6035 15.2506 10.9879 14.9132L8.10753 13.3343C8.04032 13.2975 7.95967 13.2975 7.89247 13.3343L5.0121 14.9132C4.39652 15.2506 3.74112 14.5374 4.10401 13.925L7.4145 8.3381Z"/><path d="M13.5 8C13.5 4.96243 11.0376 2.5 8 2.5C4.96243 2.5 2.5 4.96243 2.5 8C2.5 8.96927 2.75037 9.87822 3.18945 10.668L3.38867 10.999C3.60231 11.4033 3.4953 11.825 3.16992 12.0371C2.84468 12.249 2.41642 12.1766 2.17773 11.8809L2.00195 11.6104C1.36597 10.5558 1 9.31963 1 8C1 4.13401 4.13401 1 8 1C11.866 1 15 4.13401 15 8C15 9.40749 14.5834 10.7198 13.8672 11.8184C13.5836 12.1766 13.1553 12.249 12.8301 12.0371C12.4831 11.8109 12.3851 11.346 12.6113 10.999L12.8105 10.668C13.2496 9.87822 13.5 8.96927 13.5 8Z"/></svg>}
...
function SectionDisclosureIcon({expanded}:{expanded:boolean}){return <svg className={expanded?'expanded':''} viewBox="0 0 16 16" aria-hidden="true"><path d="M7.00194 10.6239C6.66861 10.8183 6.25 10.5779 6.25 10.192V5.80802C6.25 5.42212 6.66861 5.18169 7.00194 5.37613L10.7596 7.56811C11.0904 7.76105 11.0904 8.23895 10.7596 8.43189L7.00194 10.6239Z"/></svg>}
function TeamMenuIcon(){return <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M3 6.5a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm5 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Zm5 0a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3Z"/></svg>}
...
function menuMatches(query:string,label:string){return!query.trim()||label.toLowerCase().includes(query.trim().toLowerCase())}
