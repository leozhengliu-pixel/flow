...
function MoreMenu({
  ...
  > = {
    ...
    loops: { label: "Loops", icon: <LoopsIcon />, to: loopsPath(workspaceSlug) },
    documents: {
      label: "Documents",
      icon: <BookOpen />,
      to: documentsPath(workspaceSlug),
...
export function SidebarCustomization({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sidebar-customize-dialog" closeLabel={t("Close modal dialog")} overlayClassName="sidebar-customize-overlay">
        <header className="sidebar-customize-header"><DialogTitle>{t("Customize sidebar")}</DialogTitle></header>
        <div className="sidebar-customize-body">
          <div className="sidebar-badge-style">
            ...
            <SidebarSelect
              ...
              compact
              options={[{ value: "count", label: t("Count") }, { value: "dot", label: t("Dot") }]}
              value={badgeStyle}
              onChange={(value) => onBadgeStyleChange(value as SidebarBadgeStyle)}
              renderValue={(value) => <span className="sidebar-badge-value"><span className={`badge-preview is-${value}`}>{value === "count" ? "1" : ""}</span>{value === "count" ? t("Count") : t("Dot")}</span>}
            />
          </div>
          <CustomizationGroup
            group="personal"
        ...
        {entries.map(([id, name, icon]) => {
          ...
          const visibilityLabel =
            locale === "zh-CN"
              ? `${localizedName}的可见性`
              : `${name} visibility`;
          const visibilityOptions = id === "inbox"
            ? [{ value: "always", label: t("Always show") }, { value: "badged", label: t("Show when badged") }]
            : ["reviews", "pulse", "drafts"].includes(id)
              ? [{ value: "always", label: t("Always show") }, { value: "badged", label: t("Show when badged") }, { value: "never", label: t("Don't show") }]
              : [{ value: "always", label: t("Always show") }, { value: "never", label: t("Don't show") }];
          return (
            <div
...
function SidebarSelect({ ariaLabel, compact = false, onChange, options, renderValue, value }: {
  ariaLabel: string;
  compact?: boolean;
  ...
  return <Select.Root value={selected.value} onValueChange={onChange}>
    <Select.Trigger aria-label={ariaLabel} className="sidebar-customize-select">
      <Select.Value asChild><span className="sidebar-customize-select-value">{renderValue ? renderValue(selected.value) : selected.label}</span></Select.Value>
      <Select.Icon asChild><SidebarChevronIcon/></Select.Icon>
    </Select.Trigger>
    <Select.Portal>
      <Select.Content align="start" alignOffset={-2} className={`sidebar-customize-select-menu${compact ? " is-compact" : ""}`} position="popper" side="bottom" sideOffset={-35 - selectedIndex * 32}>
        <Select.Viewport>
          {options.map((option) => <Select.Item className="sidebar-customize-select-option" key={option.value} value={option.value}>
            <Select.ItemText>{option.label}</Select.ItemText>
            <Select.ItemIndicator className="sidebar-customize-select-indicator"><Check/></Select.ItemIndicator>
          </Select.Item>)}
        </Select.Viewport>
      </Select.Content>
    </Select.Portal>
...
function SidebarChevronIcon() {
  return <svg aria-hidden="true" viewBox="0 0 9 5"><path d="M1.915.557a.667.667 0 0 0-.943.943l2.862 2.862a.942.942 0 0 0 1.333 0L8.028 1.5a.667.667 0 0 0-.943-.943L4.5 3.14 1.915.557Z"/></svg>;
}

function SidebarDragHandleIcon() {
  return <svg aria-hidden="true" viewBox="0 0 6 10"><path fillRule="evenodd" clipRule="evenodd" d="M1 8C1.55228 8 2 8.44772 2 9C2 9.55228 1.55228 10 1 10C.447715 10 0 9.55228 0 9C0 8.44772.447715 8 1 8Zm4 0C5.55228 8 6 8.44772 6 9c0 .55228-.44772 1-1 1s-1-.44772-1-1c0-.55228.44772-1 1-1ZM1 4c.55228 0 1 .44772 1 1S1.55228 6 1 6 0 5.55228 0 5s.44772-1 1-1Zm4 0c.55228 0 1 .44772 1 1S5.55228 6 5 6 4 5.55228 4 5s.44772-1 1-1ZM1 0c.55228 0 1 .447715 1 1s-.44772 1-1 1S0 1.55228 0 1 .447715 0 1 0Zm4 0c.55228 0 1 .447715 1 1s-.44772 1-1 1-1-.447715-1-1 .44772-1 1-1Z"/></svg>;
}
