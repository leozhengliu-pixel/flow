...
export function InitiativeNotificationMenu({ initiative, onUpdate }: { initiative: Initiative; onUpdate: Update }) {
  ...
  return <>
    <Popover.Root><Popover.Trigger asChild><button aria-label="Setup initiative notifications" className={initiative.subscribed ? 'is-active' : ''} type="button"><Bell size={14}/></button></Popover.Trigger><Popover.Portal><Popover.Content align="end" alignOffset={-2} className="li-notifications" collisionPadding={10} sideOffset={4}>
      <NotificationSection title="Send inbox notifications for"><NotificationCheck checked={rules.descriptionChanges} label="Comments and changes to initiative description" onChange={value => changeRule('descriptionChanges', value)}/><NotificationCheck checked={rules.newUpdate} label="New initiative update is posted" onChange={value => changeRule('newUpdate', value)}/></NotificationSection>
...
export function InitiativeActionsMenu({ initiative, onCreateReminder, onDelete, onNewUpdate, onShowActivity, onUpdate }: { initiative: Initiative; onCreateReminder: (remindAt: string) => Promise<unknown>; onDelete: () => void; onNewUpdate: () => void; onShowActivity: () => void; onUpdate: Update }) {
  ...
  return <>
    ...
    <DescriptionHistoryDialog initiative={initiative} onOpenChange={setHistoryOpen} onUpdate={onUpdate} open={historyOpen}/>
    <UpdateScheduleDialog initiative={initiative} onOpenChange={setScheduleOpen} onUpdate={onUpdate} open={scheduleOpen}/>
...
function UpdateScheduleDialog({ initiative, open, onOpenChange, onUpdate }: { initiative: Initiative; open: boolean; onOpenChange: (open: boolean) => void; onUpdate: Update }) {
  ...
  return <Dialog.Root onOpenChange={onOpenChange} open={open}><Dialog.Portal><Dialog.Overlay className="li-dialog-overlay"/><Dialog.Content className="li-schedule-dialog"><Dialog.Title>Change update schedule</Dialog.Title><Dialog.Description>Choose when updates are expected for <strong data-i18n-ignore>{initiative.name}</strong>.</Dialog.Description><div className="li-schedule-options">{(['none', 'weekly', 'biweekly', 'monthly', 'custom', 'never'] as const).map(cadence => <label key={cadence}><input checked={draft.cadence === cadence} name="cadence" onChange={() => setDraft({ ...draft, cadence })} type="radio"/><span>{scheduleCadenceLabel(cadence)}</span></label>)}</div>{draft.cadence !== 'none' && draft.cadence !== 'never' && <div className="li-schedule-custom"><label>Weekday<select value={draft.weekday} onChange={event => setDraft({ ...draft, weekday: Number(event.target.value) })}>{['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'].map((day, index) => <option key={day} value={index}>{day}</option>)}</select></label><label>Time range<select value={draft.timeRange} onChange={event => setDraft({ ...draft, timeRange: event.target.value })}><option value="09:00-12:00">09:00–12:00</option><option value="12:00-15:00">12:00–15:00</option><option value="15:00-18:00">15:00–18:00</option></select></label></div>}<footer><Dialog.Close asChild><button type="button">Cancel</button></Dialog.Close><button disabled={saving} onClick={() => { setSaving(true); Promise.resolve(onUpdate({ updateSchedule: draft })).then(() => onOpenChange(false)).finally(() => setSaving(false)) }} type="button">Save</button></footer></Dialog.Content></Dialog.Portal></Dialog.Root>
}

function DescriptionHistoryDialog({ initiative, open, onOpenChange, onUpdate }: { initiative: Initiative; open: boolean; onOpenChange: (open: boolean) => void; onUpdate: Update }) {
...
function ReminderDialog({ open, onOpenChange, onCreate }: { open: boolean; onOpenChange: (open: boolean) => void; onCreate: (remindAt: string) => Promise<unknown> }) {
  ...
  const [saving, setSaving] = useState(false)
  return <Dialog.Root onOpenChange={onOpenChange} open={open}><Dialog.Portal><Dialog.Overlay className="li-dialog-overlay"/><Dialog.Content className="li-reminder-dialog"><Dialog.Title>Set reminder</Dialog.Title><Dialog.Description>Choose a date and time in your local timezone.</Dialog.Description><label>Remind me at<input autoFocus min={toLocalInput(new Date())} onChange={event => setValue(event.target.value)} type="datetime-local" value={value}/></label><footer><Dialog.Close asChild><button type="button">Cancel</button></Dialog.Close><button disabled={saving || !value || new Date(value) <= new Date()} onClick={() => { setSaving(true); onCreate(new Date(value).toISOString()).then(() => { toast.success('Reminder created'); onOpenChange(false) }).finally(() => setSaving(false)) }} type="button">Create reminder</button></footer></Dialog.Content></Dialog.Portal></Dialog.Root>
}
