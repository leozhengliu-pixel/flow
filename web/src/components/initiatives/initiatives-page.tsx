  ...
  return <main className="main-panel li-page">
    ...
    <div className="li-toolbar">
      ...
      <div className="li-toolbar-actions">
        ...
        <InitiativeFilterMenu filters={filters} initiatives={initiatives} labels={labels} onAdvanced={() => { setAdvancedFilterEnabled(true); setAdvancedFilterOpen(true) }} onChange={setFilters} onCreateLabel={onCreateLabel} teams={teams} users={users}/>
...
function InitiativeRowContextMenu({ initiative, users, teams, labels, onCreateReminder, onDelete, onOpen, onUpdate }: { initiative: Initiative; users: User[]; teams: Team[]; labels: IssueLabel[]; onCreateReminder: (remindAt: string) => Promise<unknown>; onDelete: () => void; onOpen: (tab?: InitiativeRouteTab) => void; onUpdate: (input: InitiativeMutationInput) => void | Promise<unknown> }) {
  ...
  return <ContextMenu.Content data-flow-motion="floating" className="li-menu li-row-context" onCloseAutoFocus={() => setQuery('')}>
    ...
    {visible('Owner') && <ContextMenu.Sub><ContextMenu.SubTrigger><span className="li-owner-empty">+</span>Owner<kbd>N then O</kbd><ChevronRight size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu"><PeopleMenuItems contextMenu users={users} selectedId={initiative.owner?.id ?? ''} emptyLabel="No owner" onSelect={ownerId => onUpdate({ ownerId })}/></ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    {visible('Lead team') && <ContextMenu.Sub><ContextMenu.SubTrigger><span className="li-owner-empty">+</span>Lead team<ChevronRight size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu"><ContextMenu.Item onSelect={() => onUpdate({ leadTeamId: '' })}>No lead team</ContextMenu.Item>{teams.map(team => <ContextMenu.Item key={team.id} onSelect={() => onUpdate({ leadTeamId: team.id })}><span className="li-team-mark"><ViewGlyph color={team.color} icon={team.icon || 'Team'}/></span><span data-i18n-ignore>{team.name}</span>{initiative.leadTeamId === team.id && <Check className="li-menu-end" size={12}/>}</ContextMenu.Item>)}</ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    {visible('Target date') && <ContextMenu.Sub><ContextMenu.SubTrigger><span className="li-target-glyph is-empty"/>Target date…<kbd>Ctrl ⌥ D</kbd><ChevronRight size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu"><ContextMenu.Item onSelect={() => onUpdate({ targetDate: '' })}>No target date</ContextMenu.Item><ContextMenu.Item onSelect={() => onUpdate({ targetDate: isoDate(new Date()) })}>Today</ContextMenu.Item><ContextMenu.Item onSelect={() => onUpdate({ targetDate: isoDate(due) })}>Tomorrow</ContextMenu.Item></ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    {visible('Labels') && <ContextMenu.Sub><ContextMenu.SubTrigger><span className="li-label-outline"/>Labels<kbd>N then L</kbd><ChevronRight size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu">{labels.map(label => <ContextMenu.Item key={label.id} onSelect={() => onUpdate({ labelIds: initiative.labelIds.includes(label.id) ? initiative.labelIds.filter(id => id !== label.id) : [...initiative.labelIds, label.id] })}><i className="li-filter-color" style={{ background: label.color }}/><span data-i18n-ignore>{label.name}</span>{initiative.labelIds.includes(label.id) && <Check className="li-menu-end" size={12}/>}</ContextMenu.Item>)}</ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    <ContextMenu.Separator/>
    {visible('Copy') && <ContextMenu.Sub><ContextMenu.SubTrigger><Copy size={14}/>Copy<ChevronRight className="li-menu-end" size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu"><ContextMenu.Item onSelect={() => void navigator.clipboard.writeText(location.href)}>Copy page URL</ContextMenu.Item><ContextMenu.Item onSelect={() => void navigator.clipboard.writeText(initiative.name)}>Copy initiative title</ContextMenu.Item></ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    <ContextMenu.Separator/>
    ...
    {visible('Remind me') && <ContextMenu.Sub><ContextMenu.SubTrigger><Clock3 size={14}/>Remind me<kbd>⇧ H</kbd><ChevronRight size={12}/></ContextMenu.SubTrigger><ContextMenu.Portal><ContextMenu.SubContent data-flow-motion="floating" className="li-menu"><ContextMenu.Item onSelect={() => void onCreateReminder(new Date(Date.now() + 3600000).toISOString())}>In one hour</ContextMenu.Item><ContextMenu.Item onSelect={() => void onCreateReminder(new Date(Date.now() + 86400000).toISOString())}>Tomorrow</ContextMenu.Item><ContextMenu.Item onSelect={() => void onCreateReminder(new Date(Date.now() + 604800000).toISOString())}>Next week</ContextMenu.Item></ContextMenu.SubContent></ContextMenu.Portal></ContextMenu.Sub>}
    <ContextMenu.Separator/>
...
export function InitiativeFilterMenu({ filters, initiatives, users, teams, labels, onChange, onCreateLabel, onAdvanced }: { filters: FilterState; initiatives: Initiative[]; users: User[]; teams: Team[]; labels: IssueLabel[]; onChange: (filters: FilterState) => void; onCreateLabel?: (name: string) => Promise<IssueLabel>; onAdvanced: () => void }) {
  const [query, setQuery] = useState('')
  const entries = [
...
function InitiativeFilterValues({ entry, filters, initiatives, labels, onChange, onCreateLabel, teams, users }: { entry: { id: string; label: string }; filters: FilterState; initiatives: Initiative[]; labels: IssueLabel[]; onChange: (filters: FilterState) => void; onCreateLabel?: (name: string) => Promise<IssueLabel>; teams: Team[]; users: User[] }) {
  const options = useMemo(() => filterOptions(entry.id, initiatives, users, teams, labels).map(option => ({ ...option, id: String(option.value), entity: ['ownerId','creatorId','leadTeamId','teamId','labelId'].includes(entry.id) })), [entry.id, initiatives, users, teams, labels])
  const counts = useMemo(() => entry.id === 'date' ? new Map(options.map(option => [option.id, initiatives.filter(item => matchesDateFilter(item, option.value as NonNullable<FilterState['date']>)).length])) : countInitiativeFilterValues(initiatives, entry.id), [entry.id, initiatives, options])
  return <SearchableMenuItems options={options} onSelect={option => onChange({ ...filters, [entry.id]: option.value })} onCreate={entry.id === 'labelId' && onCreateLabel ? async name => { const label = await onCreateLabel(name); onChange({ ...filters, labelId: label.id }) } : undefined} renderOption={option => <><span className="li-filter-checkbox">{filters[entry.id as keyof FilterState] === option.value && <Check size={10}/>}</span>{option.icon}<span data-menu-label data-i18n-ignore={option.entity || undefined}>{option.label}</span>{Boolean(counts.get(option.id)) && <small className="li-menu-end">{counts.get(option.id)} initiative{counts.get(option.id) === 1 ? '' : 's'}</small>}</>}/>
}
...
function AdvancedFilterBar(props: AdvancedFilterProps & { open: boolean; onOpenChange: (open: boolean) => void; onRemove: () => void }) {
  ...
  return <div className="li-filter-chips li-advanced-filter-bar">
    <Popover.Root onOpenChange={onOpenChange} open={open}>
      ...
      <Popover.Portal><Popover.Content data-flow-motion="floating" align="start" className="li-advanced-filter-popover" collisionPadding={8} side="bottom" sideOffset={4} onOpenAutoFocus={event => event.preventDefault()}>
...
function AdvancedFilterRule({ field, value, onRemove, onReplaceField, ...props }: AdvancedFilterProps & { field: keyof FilterState; value: NonNullable<FilterState[keyof FilterState]>; onRemove: () => void; onReplaceField: (field: keyof FilterState) => void }) {
  ...
  return <div className="li-advanced-filter-rule">
    <DropdownMenu.Root><DropdownMenu.Trigger asChild><button type="button">{fieldLabel}</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className="li-menu li-advanced-filter-menu" sideOffset={4}>{ADVANCED_FILTER_FIELDS.map(item => <DropdownMenu.Item disabled={item.id !== field && props.filters[item.id] !== undefined} key={item.id} onSelect={() => onReplaceField(item.id)}>{item.label}{item.id === field && <Check className="li-menu-end" size={12}/>}</DropdownMenu.Item>)}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
    <span>is</span>
    <DropdownMenu.Root><DropdownMenu.Trigger asChild><button data-i18n-ignore={['ownerId','creatorId','leadTeamId','teamId','labelId'].includes(field) ? '' : undefined} type="button">{valueOption?.label ?? String(value)}</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className={`li-menu li-filter-values is-${field}`} sideOffset={4} collisionPadding={8}><InitiativeFilterValues entry={{id:field,label:fieldLabel}} filters={props.filters} initiatives={props.initiatives} labels={props.labels} onChange={props.onChange} onCreateLabel={props.onCreateLabel} teams={props.teams} users={props.users}/></DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
...
function AdvancedFilterPicker(props: AdvancedFilterProps) {
  const { t } = useI18n()
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button aria-label={t('Filter')} className="li-advanced-filter-picker" disabled={Object.keys(props.filters).length >= ADVANCED_FILTER_FIELDS.length} type="button"><Plus size={12}/>Filter</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className="li-menu li-advanced-filter-menu" sideOffset={4}>{ADVANCED_FILTER_FIELDS.map(field => <DropdownMenu.Sub key={field.id}><DropdownMenu.SubTrigger disabled={props.filters[field.id] !== undefined}>{field.label}<ChevronRight className="li-menu-end" size={12}/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent data-flow-motion="floating" className={`li-menu li-filter-values is-${field.id}`} sideOffset={4} collisionPadding={8}><InitiativeFilterValues entry={field} filters={props.filters} initiatives={props.initiatives} labels={props.labels} onChange={props.onChange} onCreateLabel={props.onCreateLabel} teams={props.teams} users={props.users}/></DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub>)}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
}
...
function filterOptions(id: string, initiatives: Initiative[], users: User[], teams: Team[], labels: IssueLabel[]) {
  ...
  if (id === 'labelId') return labelsForResource(labels, 'initiative').map(label => ({ value: label.id, label: label.name, icon: <i className="li-filter-color" style={{ background: label.color }}/> }))
  ...
  if (id === 'leadTeamId' || id === 'teamId') return teams.map(team => ({ value: team.id, label: team.name, icon: <span className="li-team-mark"><ViewGlyph color={team.color} icon={team.icon || 'Team'}/></span> }))
