...
import type { Initiative, InitiativeMutationInput, InitiativeStatus, InitiativeUpdate, IssueLabel, Project, ProjectUpdate, User } from '@/types/flow'
...
const PROPERTY_ORDER = ['description', 'owner', 'status', 'priority', 'teams', 'health', 'projects', 'activeProjects', 'target', 'created', 'updated', 'completed', 'labels'] as const
type Property = typeof PROPERTY_ORDER[number]
const TABLE_PROPERTY_ORDER: Property[] = ['status', 'priority', 'owner', 'target', 'projects', 'health', 'activeProjects', 'labels', 'teams', 'created', 'updated', 'completed']
const DEFAULT_PROPERTIES: Property[] = ['description', 'owner', 'priority', 'health', 'projects', 'activeProjects', 'target']
type Sort = 'manual' | 'name' | 'priority' | 'target' | 'health' | 'created' | 'updated'
type FilterState = { status?: InitiativeStatus; priority?: number; ownerId?: string; health?: Project['health']; labelId?: string }

export function InitiativesPage(props: Props) {
  const { initiatives, initiativeUpdates, projects, projectUpdates, users, labels, viewer, view, onViewChange, onOpen, onCreate, onCreateUpdate, onUpdate, onDelete, onOpenSidebar, createOnMount = false } = props
  ...
  const [detailsOpen, setDetailsOpen] = useState(() => localStorage.getItem('flow:initiatives:details-open') !== 'false')
  ...
  return <main className="main-panel li-page">
    ...
    <div className="li-toolbar">
      ...
      <div className="li-toolbar-actions">
        <FilterMenu filters={filters} initiatives={initiatives} labels={labels} onChange={setFilters} users={users}/>
        <DisplayMenu properties={properties} sort={sort} onProperty={toggleProperty} onSort={setSort}/>
...
function InitiativesListSidebar({ initiatives, users, view }: { initiatives: Initiative[]; users: User[]; view: InitiativesRouteView }) {
  const [tab, setTab] = useState<'owner' | 'health'>('owner')
  ...
  return <aside className="li-list-sidebar"><div className="li-list-sidebar-tabs" role="tablist"><button aria-selected={tab === 'owner'} onClick={() => setTab('owner')} role="tab" type="button">Owner</button><button aria-selected={tab === 'health'} onClick={() => setTab('health')} role="tab" type="button">Health</button></div><div className="li-list-sidebar-items">{tab === 'owner' ? ownerCounts.map(({ user, count }) => <button key={user.id} type="button"><Avatar name={user.displayName || user.name}/><span className="li-list-sidebar-label">{user.displayName || user.name}</span><small>{count}</small></button>) : healthCounts.map(({ health, count }) => <button key={health} type="button"><i className={`li-update-health is-${health}`}/><span className="li-list-sidebar-label">{healthLabel(health)}</span><small>{count}</small></button>)}</div></aside>
}
...
function InitiativeRow({ initiative, initiativeUpdates, projects, projectUpdates, properties, columns, selected, users, labels, onDelete, onOpen, onOpenUpdates, onSelect, onUpdate }: {
  initiative: Initiative; initiativeUpdates: InitiativeUpdate[]; projects: Project[]; projectUpdates: Record<string, ProjectUpdate[]>; properties: Set<Property>; columns: Property[]; selected: boolean; users: User[]; labels: IssueLabel[]
  onDelete: (id: string) => Promise<void>; onOpen: (initiative: Initiative, tab?: InitiativeRouteTab) => void; onOpenUpdates: () => void; onSelect: () => void; onUpdate: (input: InitiativeMutationInput) => void | Promise<unknown>
}) {
  ...
  return <ContextMenu.Root><ContextMenu.Trigger asChild><div className="li-row" data-selected={selected} onClick={() => onOpen(initiative)}>
    ...
    <div className="li-name"><strong>{initiative.name}</strong>{properties.has('description') && <small>{initiative.summary}</small>}</div>
  ...
  </div></ContextMenu.Trigger><ContextMenu.Portal><InitiativeRowContextMenu initiative={initiative} labels={labels} users={users} onDelete={() => onDelete(initiative.id)} onOpen={tab => onOpen(initiative, tab)} onUpdate={onUpdate}/></ContextMenu.Portal></ContextMenu.Root>

  function renderCell(property: Property) {
    if (property === 'priority') return <InitiativeProperties compact only="priority" initiative={initiative} users={users} onUpdate={onUpdate}/>
    if (property === 'owner') return <InitiativeProperties compact only="owner" initiative={initiative} users={users} onUpdate={onUpdate}/>
    if (property === 'target') return <InitiativeProperties compact only="target" initiative={initiative} users={users} onUpdate={onUpdate}/>
    if (property === 'status') return <InitiativeProperties compact only="status" initiative={initiative} users={users} onUpdate={onUpdate}/>
    ...
    if (property === 'teams') return <span className="li-muted">Cleantrack</span>
...
function InitiativeRowContextMenu({ initiative, users, labels, onDelete, onOpen, onUpdate }: { initiative: Initiative; users: User[]; labels: IssueLabel[]; onDelete: () => void; onOpen: (tab?: InitiativeRouteTab) => void; onUpdate: (input: InitiativeMutationInput) => void | Promise<unknown> }) {
  ...
  return <ContextMenu.Content className="li-menu li-row-context" onCloseAutoFocus={() => setQuery('')}>
    ...
    {visible('Remind me') && <ContextMenu.Item onSelect={() => toast.success('Reminder set for tomorrow')}><Clock3 size={14}/>Remind me<kbd>⇧ H</kbd><ChevronRight size={12}/></ContextMenu.Item>}
    <ContextMenu.Separator/>
...
function InitiativeCreateRow({ labels, users, view, onCancel, onCreate }: { labels: IssueLabel[]; users: User[]; view: InitiativesRouteView; onCancel: () => void; onCreate: (input: InitiativeMutationInput & { name: string }) => Promise<void> }) {
  const initialStatus: InitiativeStatus = view === 'planned' ? 'planned' : 'active'
  const [draft, setDraft] = useState<Initiative>({ id: 'draft', name: '', slugId: '', summary: '', description: '', icon: 'Initiative', color: '#8a8f98', status: initialStatus, priority: 0, priorityLabel: 'No priority', health: 'noUpdate', labelIds: [], projectIds: [], resources: [], comments: [], favorite: false, subscribed: false, createdAt: '', updatedAt: '' })
  ...
  const submit = async () => {
    ...
    setSaving(true)
    try { await onCreate({ name: draft.name.trim(), summary: draft.summary, icon: draft.icon, color: draft.color, status: draft.status, priority: draft.priority, ownerId: draft.owner?.id, targetDate: draft.targetDate, labelIds: draft.labelIds }) } finally { setSaving(false) }
  }
...
function ColumnHeader({ property, onSort }: { property: Property; onSort: (sort: Sort) => void }) {
  const label = ({ owner: 'Owner', status: 'Status', priority: 'Priority', teams: 'Contributing teams', health: 'Health', projects: 'Projects', activeProjects: 'Active Projects', target: 'Target', created: 'Created', updated: 'Updated', completed: 'Completed', labels: 'Labels', description: 'Description' } as const)[property]
...
function DisplayMenu({ properties, sort, onProperty, onSort }: { properties: Set<Property>; sort: Sort; onProperty: (property: Property) => void; onSort: (sort: Sort) => void }) {
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button aria-label="Display options" className="li-icon-button" type="button"><SlidersHorizontal size={14}/></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content align="end" className="li-menu li-display-menu" sideOffset={4}>
    <div className="li-display-row"><span>Grouping</span><DropdownMenu.Sub><DropdownMenu.SubTrigger>No grouping <ChevronRight size={12}/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent className="li-menu"><DropdownMenu.Item>No grouping</DropdownMenu.Item><DropdownMenu.Item>Status</DropdownMenu.Item><DropdownMenu.Item>Owner</DropdownMenu.Item><DropdownMenu.Item>Health</DropdownMenu.Item></DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub></div>
    ...
    <DropdownMenu.Separator/><DropdownMenu.Label>Display properties</DropdownMenu.Label><div className="li-display-properties">{PROPERTY_ORDER.map(property => <button aria-pressed={properties.has(property)} className={properties.has(property) ? 'is-active' : ''} key={property} onClick={event => { event.preventDefault(); onProperty(property) }} type="button">{columnLabel(property)}</button>)}</div>
  </DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
}

function FilterMenu({ filters, initiatives, users, labels, onChange }: { filters: FilterState; initiatives: Initiative[]; users: User[]; labels: IssueLabel[]; onChange: (filters: FilterState) => void }) {
  const [query, setQuery] = useState('')
  const entries = [
    { id: 'status', label: 'Status' }, { id: 'priority', label: 'Priority' }, { id: 'ownerId', label: 'Owner' }, { id: 'creator', label: 'Creator' },
    { id: 'teams', label: 'Contributing teams' }, { id: 'labelId', label: 'Labels' }, { id: 'health', label: 'Health' }, { id: 'dates', label: 'Dates' },
  ...
  return <DropdownMenu.Root onOpenChange={open => { if (!open) setQuery('') }}><DropdownMenu.Trigger asChild><button aria-label="Add filter" className="li-icon-button" type="button"><Filter size={14}/>{Object.keys(filters).length > 0 && <i/>}</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content align="end" className="li-menu li-filter-menu" sideOffset={4}>
    ...
    {!query && <><DropdownMenu.Item onSelect={() => toast.info('Describe the initiatives you want to filter with AI.')}><span className="li-ai-filter">✦</span>AI filter</DropdownMenu.Item><DropdownMenu.Separator/><DropdownMenu.Item onSelect={() => toast.info('Advanced filters can combine the properties below.')}>Advanced filter</DropdownMenu.Item><DropdownMenu.Separator/></>}
    {entries.map(entry => <DropdownMenu.Sub key={entry.id}><DropdownMenu.SubTrigger>{entry.label}<ChevronRight className="li-menu-end" size={13}/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent className="li-menu li-filter-values" sideOffset={5}>{filterOptions(entry.id, initiatives, users, labels).map(option => <DropdownMenu.Item key={String(option.value)} onSelect={() => { if (entry.id === 'creator' || entry.id === 'teams' || entry.id === 'dates') { toast.info(`${entry.label} is not available in the current workspace data.`); return } onChange({ ...filters, [entry.id]: option.value }) }}>{option.icon}{option.label}{filters[entry.id as keyof FilterState] === option.value && <Check className="li-menu-end" size={13}/>}</DropdownMenu.Item>)}</DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub>)}
  </DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
}
...
function filterOptions(id: string, initiatives: Initiative[], users: User[], labels: IssueLabel[]) {
  ...
  if (id === 'ownerId' || id === 'creator') return [{ value: '', label: 'No owner', icon: <span className="li-owner-empty">+</span> }, ...users.map(user => ({ value: user.id, label: user.displayName, icon: <Avatar name={user.displayName}/> }))]
  ...
  if (id === 'teams') return [{ value: 'cleantrack', label: 'Cleantrack', icon: <span/> }]
  if (id === 'dates') return ['Created', 'Updated', 'Target date', 'Completed'].map(value => ({ value, label: value, icon: <span/> }))
  return initiatives.map(item => ({ value: item.id, label: item.name, icon: <span/> }))
}

function columnLabel(property: Property) { return ({ description: 'Description', owner: 'Owner', status: 'Status', priority: 'Priority', teams: 'Contributing teams', health: 'Health', projects: 'Projects', activeProjects: 'Active projects', target: 'Target date', created: 'Created', updated: 'Updated', completed: 'Completed', labels: 'Labels' } as const)[property] }
function filterLabel(key: string, value: unknown, users: User[], labels: IssueLabel[]) { if (key === 'ownerId') return `Owner: ${users.find(user => user.id === value)?.displayName ?? 'No owner'}`; if (key === 'labelId') return `Labels: ${labels.find(label => label.id === value)?.name ?? value}`; if (key === 'priority') return `Priority: ${['No priority', 'Urgent', 'High', 'Medium', 'Low'][Number(value)]}`; return `${titleCase(key)}: ${titleCase(String(value))}` }
