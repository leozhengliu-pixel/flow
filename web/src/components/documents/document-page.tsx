...
import { issuePath, projectPath, teamDocumentsPath, teamHomePath } from '@/lib/app-routes'
  ...
  return <main className="main-panel document-page">
    <header className="document-header">
      <nav aria-label="Document breadcrumb" className="document-breadcrumbs">
        {issue?<a className="document-breadcrumb-link" href={issuePath(data.workspace.urlKey,issue)}><FileText/><span data-i18n-ignore>{issue.identifier}</span></a>:project?<a className="document-breadcrumb-link" href={projectPath(data.workspace.urlKey,project)}><FileText style={{color:project.color}}/><span data-i18n-ignore>{project.name}</span></a>:team?<><a className="document-breadcrumb-link" href={teamHomePath(data.workspace.urlKey,team.key)}><FileText style={{color:team.color}}/><span data-i18n-ignore>{team.name}</span></a><span aria-hidden="true" className="document-breadcrumb-separator">›</span><a className="document-breadcrumb-link document-breadcrumb-documents" href={teamDocumentsPath(data.workspace.urlKey,team.key)}>Documents</a></>:<button className="document-breadcrumb-link" onClick={onBack}><FileText/><span>Documents</span></button>}
        <span aria-hidden="true" className="document-breadcrumb-separator">›</span><strong className="document-breadcrumb-current"><ViewGlyph color={document.color||'#8b8b90'} icon={document.icon||'Page'}/><span data-i18n-ignore>{document.title}</span></strong>
      </nav>
      <div className="document-header-actions">
        {saveState==='error'&&<span className="document-save-state error">Could not save</span>}
        {!defaultDocumentIcon&&<button aria-label={favorite?'Remove from favorites':'Add to favorites'} onClick={()=>void (favorite?removeFavorite('document',document.id):addFavorite('document',document.id)).then(onReload)}><Star size={15} fill={favorite?'currentColor':'none'}/></button>}
        ...
        <button aria-label="Copy document URL" onClick={()=>void navigator.clipboard.writeText(location.href)}><Link2 size={15}/></button>
    ...
    <article className="document-canvas">
      <ViewIconPicker ariaLabel="Document icon" color={document.color || '#8b8b90'} icon={document.icon || 'Page'} onChange={visual=>void updateDocument(document.id,visual).then(onReload)} triggerClassName={`document-icon${defaultDocumentIcon?' is-empty':''}`}/>
