  ...
  return <main className="main-panel document-page">
    <header className="document-header">
      ...
      <div className="document-header-actions">
        ...
        <DropdownMenu.Root><DropdownMenu.Trigger asChild><button aria-label="Document options"><MoreHorizontal size={16}/></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content className="document-menu" align="end" sideOffset={5}><DropdownMenu.Sub><DropdownMenu.SubTrigger><FileText size={14}/><span>Move to</span><ChevronDown size={12}/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent className="document-menu" sideOffset={6}>{data.projects.map(item=><DropdownMenu.CheckboxItem checked={document.projectIds.includes(item.id)} key={item.id} onCheckedChange={()=>void toggleProject(item.id)} onSelect={event=>event.preventDefault()}><i style={{background:item.color}}/><span data-i18n-ignore>{item.name}</span>{document.projectIds.includes(item.id)&&<Check size={13}/>}</DropdownMenu.CheckboxItem>)}</DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub><DropdownMenu.Item onSelect={()=>navigator.clipboard.writeText(location.href)}><Copy size={14}/><span>Copy link</span></DropdownMenu.Item>{canManageAccess&&<DropdownMenu.Item onSelect={()=>void openAccess()}><Users size={14}/><span>People with access</span></DropdownMenu.Item>}<DropdownMenu.Item onSelect={()=>void openHistory()}><History size={14}/><span>Show document history</span></DropdownMenu.Item><DropdownMenu.Separator/><DropdownMenu.Item className="danger" onSelect={()=>setDeleteOpen(true)}><Trash2 size={14}/><span>Delete</span></DropdownMenu.Item></DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
