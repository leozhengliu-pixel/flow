...
import { Bell, Building2, ChevronDown, ChevronRight, Copy, Download, Ellipsis, Layers3, LockKeyhole, Pencil, Trash2, UserRound } from 'lucide-react'
...
export function SavedViewEditor({ actions, ariaLabel = 'New view', initialName = '', namePlaceholder = 'All issues', initialDescription = '', initialIcon = DEFAULT_VIEW_ICON, initialColor = DEFAULT_VIEW_COLOR, initialTarget, saveTargets = [], saving = false, onCancel, onSave }: { actions?: ReactNode; ariaLabel?: string; initialName?: string; namePlaceholder?: string; initialDescription?: string; initialIcon?: string; initialColor?: string; initialTarget?: SavedViewTarget; saveTargets?: SavedViewTarget[]; saving?: boolean; onCancel: () => void; onSave: (name: string, description: string, target: SavedViewTarget | undefined, visual: ViewVisual) => void }) {
  ...
  const resolvedName = name.trim() || t(namePlaceholder)
  const save = () => { if (!saving) onSave(resolvedName, description.trim(), target, visual) }
  ...
  return <section className={`${styles.editor} ${actions ? styles.withActions : ''}`} aria-label={t(ariaLabel)}>
    <ViewIconPicker color={visual.color} icon={visual.icon} onChange={setVisual} triggerClassName={styles.icon}/>
    <input className={styles.name} aria-label={t('View name')} autoFocus placeholder={t(namePlaceholder)} value={name} onChange={event => setName(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.nativeEvent.isComposing) save(); if (event.key === 'Escape') onCancel() }}/>
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button type="button" disabled={saving} aria-label={`${t('Save to')} ${targetLabel(target)}`}>
            ...
            <span>{targetLabel(target)}</span><ChevronDown size={11}/>
          </button>
    ...
    <button className={styles.save} type="button" disabled={saving} onClick={save}>{t(saving ? 'Saving...' : 'Save')}</button>
    <input className={styles.description} aria-label={t('Description (optional)')} placeholder={t('Description (optional)')} value={description} onChange={event => setDescription(event.target.value)}/>
