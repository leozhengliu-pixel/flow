...
export function explorerPropertyOptions(data: BootstrapData, issues = data.issues) {
  ...
  return {
    ...
    labels: data.labels.map(label => ({ id: label.id, label: label.name, color: label.color, count: count(issue => issue.labels.some(item => item.id === label.id)), kind: 'labels' as const })),
