...
function issueMatchesProjectProperties(issue:Issue,values:string[],data?:BootstrapData){const project=data?.projects.find(project=>project.id===issue.project?.id);if(!project)return false;return values.some(value=>value.startsWith('project-status:')?project.status.id===value.slice(15):value.startsWith('project-status-type:')?project.status.type===value.slice(20):value.startsWith('project-priority:')?String(project.priority)===value.slice(17):value.startsWith('project-label:')?project.labelIds.includes(value.slice(14)):value.startsWith('project-lead:')?project.lead?.id===value.slice(13):value.startsWith('project-milestone:')?project.milestones.some(milestone=>milestone.name===value.slice(18)):false)}
...
function projectPropertyFilterOptions(data:BootstrapData,issues:Issue[]):MyIssuesFilterOption[]{const projects=data.projects;const issueProject=(issue:Issue)=>projects.find(project=>project.id===issue.project?.id);const count=(predicate:(issue:Issue)=>boolean)=>issues.filter(predicate).length;return[
  {id:'project-status',label:'Project status',children:data.projectStatuses.map(status=>({id:`project-status:${status.id}`,label:status.name,color:status.color,count:count(issue=>issueProject(issue)?.status.id===status.id)}))},
  {id:'project-status-type',label:'Project status type',children:uniqueStrings(projects.map(project=>project.status.type)).map(type=>({id:`project-status-type:${type}`,label:type,count:count(issue=>issueProject(issue)?.status.type===type)}))},
  {id:'project-priority',label:'Project priority',children:['0','1','2','3','4'].map(priority=>({id:`project-priority:${priority}`,label:['No priority','Urgent','High','Medium','Low'][Number(priority)],count:count(issue=>String(issueProject(issue)?.priority)===priority)}))},
  {id:'project-labels',label:'Project labels',children:labelsForResource(data.labels,'project').map(label=>({id:`project-label:${label.id}`,label:label.name,color:label.color,count:count(issue=>issueProject(issue)?.labelIds?.includes(label.id)??false)}))},
  {id:'project-lead',label:'Project lead',children:data.users.filter(user=>user.active).map(user=>({id:`project-lead:${user.id}`,label:user.displayName,count:count(issue=>issueProject(issue)?.lead?.id===user.id)}))},
  {id:'project-milestone',label:'Project milestone name',children:uniqueStrings(projects.flatMap(project=>project.milestones?.map(milestone=>milestone.name)??[])).map(name=>({id:`project-milestone:${name}`,label:name,count:count(issue=>issueProject(issue)?.milestones?.some(milestone=>milestone.name===name)??false)}))},
]}
