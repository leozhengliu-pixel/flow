...
import type { Issue, Project, ProjectProgressHistoryPoint } from '@/types/flow'

export type ProgressSeries = { id: 'Scope' | 'Started' | 'Completed' | 'Target'; data: { x: Date; y: number }[] }
...
export function shouldShowProgressGraph(project: Pick<Project, 'startDate' | 'scopeHistory' | 'completedScopeHistory'> & Partial<Pick<Project, 'status'>>) {
  if (!project.startDate) return false
  if (project.status?.type === 'planned') return false
  const scope = project.scopeHistory ?? []
  const completed = project.completedScopeHistory ?? []
  return scope.length > 0 && scope.length === completed.length && scope.some(point => point.value > 0)
}
...
export function buildProgressData(issues: Issue[], start?: string, target?: string, persisted?: PersistedProgressHistory) {
  ...
  const completed = activeDays.map(date => ({ x: date, y: scopedIssues.filter(issue => isCompleted(issue, date)).reduce((total, issue) => total + issueEstimate(issue), 0) }))
  const completedCount = completed.at(-1)?.y ?? 0
  const historyDays = differenceInCalendarDays(currentDate, startDate)
  const inProgress = scopedIssues.filter(issue => issue.state.type === 'started' && isCreated(issue, currentDate)).reduce((total, issue) => total + issueEstimate(issue), 0)
  ...
  const remainingPoints = Math.max(0, totalEstimate - progressEstimatePoints)
  ...
  const scope = historySeries(persisted?.scopeHistory ?? persisted?.issueCountHistory, startDate, endDate) ?? finalEventDays.map(date => ({ x: date, y: scopedIssues.filter(issue => isCreated(issue, date)).reduce((total, issue) => total + issueEstimate(issue), 0) }))
  const targetData = finalEventDays.map(date => ({ x: date, y: totalEstimate }))
  const finalActiveDays = finalEventDays.filter(date => date <= currentDate)
  const completedData = historySeries(persisted?.completedScopeHistory, startDate, currentDate) ?? finalActiveDays.map(date => ({ x: date, y: scopedIssues.filter(issue => isCompleted(issue, date)).reduce((total, issue) => total + issueEstimate(issue), 0) }))
  const startedHistory = historySeries(persisted?.inProgressScopeHistory, startDate, currentDate)
...
function historyValues(entries: ProjectProgressHistoryPoint[] | undefined, startDate: Date, endDate: Date) {
  return (entries ?? []).map(entry => ({ date: normalizedHistoryDate(entry.date), value: entry.value })).filter(entry => entry.date && entry.date >= startDate && entry.date <= endDate).sort((left, right) => left.date!.getTime() - right.date!.getTime()).map(entry => entry.value)
}

function historySeries(entries: ProjectProgressHistoryPoint[] | undefined, startDate: Date, endDate: Date) {
  const points = (entries ?? []).map(entry => ({ x: normalizedHistoryDate(entry.date), y: entry.value })).filter((point): point is { x: Date; y: number } => point.x !== undefined && point.x >= startDate && point.x <= endDate).sort((left, right) => left.x.getTime() - right.x.getTime())
...
function normalizedHistoryDate(value: string) {
  const date = new Date(value)
  return Number.isFinite(date.getTime()) ? startOfDay(date) : undefined
}

function issueEstimate(issue: Issue) {
  return typeof issue.estimate === 'number' && issue.estimate > 0 ? issue.estimate : 1
}
...
function buildCompletionChanges(issues: Issue[], startDate: Date, currentDate: Date) {
  ...
  for (const issue of issues) {
    ...
    buckets.set(bucketDate.getTime(), (buckets.get(bucketDate.getTime()) ?? 0) + issueEstimate(issue))
  }
