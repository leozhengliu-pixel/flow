...
function InsightControl({ label, onChange, value }: { label: string; onChange?: (value: SegmentDimension) => void; value: string }) {
  ...
  return <label><span>{label}</span>{onChange ? <DropdownMenu.Root><DropdownMenu.Trigger asChild><button type="button">{dimensionLabel(value as SegmentDimension)}</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" align="start" className="project-detail-page__menu" sideOffset={4}>{options.map(item => <DropdownMenu.Item key={item} onSelect={() => onChange(item)}><span>{dimensionLabel(item)}</span>{item === value && <Check size={13}/>}</DropdownMenu.Item>)}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root> : <strong>Issue count</strong>}</label>
}
