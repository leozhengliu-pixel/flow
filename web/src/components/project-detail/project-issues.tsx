...
export function ProjectIssues({ display, filters, issues, labels, labelGroups, milestoneScope, onClearMilestoneScope, onCreateIssue, onDeleteIssues, onFiltersChange, onOpenIssue, onUpdateIssue, project, projects, projectIssues, users }: ProjectDetailProps & { projectIssues: Issue[]; filters: ProjectIssueFilters; display: MyIssuesDisplayOptions; milestoneScope?: ProjectMilestone; onClearMilestoneScope?: () => void; onFiltersChange: (filters: ProjectIssueFilters) => void }) {
  ...
  const changeProperty = async (row: MyIssuesRowData, property: MyIssuesEditableProperty, value: string | string[]) => {
    ...
    else if (property === 'project') await onUpdateIssue(row.id, { projectId: String(value) })
  }
...
function toRowData(issue: Issue,issues:Issue[]): MyIssuesRowData { return { id: issue.id, identifier: issue.identifier, title: issue.title, href: `/${location.pathname.split('/')[1]}/issue/${issue.identifier}`, priority: issue.priority as 0|1|2|3|4, state: issue.state, labels: issue.labels, project: issue.project, assignee: issue.assignee ? { id: issue.assignee.id, name: issue.assignee.displayName, avatarUrl: issue.assignee.avatarUrl } : undefined, estimate: issue.estimate, dueDate: issue.dueDate, createdAt: issue.createdAt, updatedAt: issue.updatedAt, parentId: issue.parentId,...issueHierarchyFields(issue,issues), sortOrder: issue.sortOrder } }
