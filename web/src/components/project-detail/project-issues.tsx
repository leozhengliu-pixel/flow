...
export function ProjectNewView({ display, filters, onCreateSavedView, onDisplayChange, onFiltersChange, onTabChange, project, projectIssues, ...props }: ProjectDetailProps & { projectIssues: Issue[]; filters: ProjectIssueFilters; display: MyIssuesDisplayOptions; onFiltersChange: (filters: ProjectIssueFilters) => void; onDisplayChange: (display: MyIssuesDisplayOptions) => void }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      await onCreateSavedView({ name: name.trim() || 'All issues', description: '', resource: 'issues', scope: 'workspace', view: 'all', ...visual, filters: [{ field: 'project', operator: 'is', values: [project.id] }, ...filters.map(filterSnapshot)], display: displaySnapshot(display) })
...
function filterSnapshot(filter: MyIssuesAppliedFilter): Record<string, unknown> { return { field: filter.field, operator: filter.operator, values: filterValues(filter).map(value => value.value) } }
