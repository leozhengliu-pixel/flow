    ...
    <section className="project-overview__intro">
      ...
      <div className="project-overview__property-section">
        ...
        <div className="project-overview__properties">
          ...
          <PropertyMenu compact label="Lead" value={project.lead?.displayName ?? 'Lead'} valueIsEntityName={Boolean(project.lead)} selectedId={project.lead?.id ?? ''} icon={project.lead ? <Avatar name={project.lead.displayName}/> : <NoAssigneeIcon size={14}/>} options={[{ id: '', label: 'No lead', icon: <NoAssigneeIcon size={14}/> }, ...users.filter(user => user.active).map(user => ({ id: user.id, label: user.displayName, keywords: `${user.name} ${user.email}`, icon: <Avatar name={user.displayName}/>, i18nIgnore: true }))]} onChange={leadId => void save({ leadId })}/>
          <PropertyMenu compact multiple label="Members" value={members.length === 1 ? members[0].displayName : members.length ? `${members.length} members` : 'Members'} valueIsEntityName={members.length === 1} selectedIds={project.memberIds ?? []} icon={members[0] ? <Avatar name={members[0].displayName}/> : <MembersIcon size={14}/>} options={users.filter(user => user.active).map(user => ({ id: user.id, label: user.displayName, icon: <Avatar name={user.displayName}/>, i18nIgnore: true }))} onChange={memberId => void save({ memberIds: (project.memberIds ?? []).includes(memberId) ? project.memberIds.filter(id => id !== memberId) : [...(project.memberIds ?? []), memberId] })}/>
...
function ProjectMoreMenu({ initiatives, labels, labelGroups, project, projects, save }: { initiatives: Props['initiatives']; labels: Props['labels']; labelGroups: Props['labelGroups']; project: Props['project']; projects: Props['projects']; save: Props['save'] }) {
  ...
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button aria-label="More project properties" className="project-overview__more" type="button">•••</button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content align="start" className="project-detail-page__menu project-overview__more-menu" sideOffset={4}>
    ...
    <DropdownMenu.Separator/><DropdownMenu.Item aria-disabled="true" disabled><SlackIcon size={14}/><span>Connect existing Slack channel…</span></DropdownMenu.Item>
  </DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
}
...
function InitiativeSection({ initiatives, project, save }: { initiatives: Props['initiatives']; project: Props['project']; save: Props['save'] }) {
  const options = initiatives.map(initiative => ({ id: initiative.id, label: initiative.name, icon: <Flag size={13}/>, i18nIgnore: true }))
  ...
  return <section className="project-overview__row-section"><h3>Initiatives</h3><div className="project-overview__row-content project-overview__initiatives">
    ...
    <PropertyMenu compact multiple label="Initiatives" value="Add initiative…" selectedIds={project.initiatives ?? []} options={options} icon={<Plus size={13}/>} onChange={id => void save({ initiatives: toggleString(project.initiatives ?? [], id) })}/>
  </div></section>
}
