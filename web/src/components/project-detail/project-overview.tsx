    ...
    <section className="project-overview__intro">
      ...
      <div className="project-overview__property-section">
        ...
        <div className="project-overview__properties">
          <PropertyMenu compact label="Status" value={project.status.name} selectedId={project.status.id} icon={<ProjectStatusIcon color={project.status.color} name={project.status.name} size={14} type={project.status.type}/>} options={statuses.map((status, index) => ({ id: status.id, label: status.name, color: status.color, icon: <ProjectStatusIcon color={status.color} name={status.name} size={14} type={status.type}/>, shortcut: String(index + 1) }))} searchPlaceholder="Change status…" searchShortcut="P, then S" surfaceClassName="project-details-sidebar__property-menu is-standard" onChange={statusId => void save({ statusId })}/>
