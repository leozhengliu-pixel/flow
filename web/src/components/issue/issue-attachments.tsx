import { File, FileImage, Link2, LoaderCircle, Paperclip, RotateCcw, Trash2 } from 'lucide-react'
...
export function IssueAttachments({ attachments, upload, onRetry, onDelete }: { attachments: Attachment[]; upload?: AttachmentUploadState; onRetry: (file: File) => void; onDelete: (id: string) => void }) {
  ...
  return <section className="issue-attachments"><header><strong>Attachments</strong><span>{attachments.length}</span></header><div className="attachment-grid">
    {attachments.map(attachment => { const image=attachment.contentType.startsWith('image/'),link=attachment.contentType==='text/uri-list'; return <article className="attachment-card" key={attachment.id}>{image ? <a className="attachment-preview" href={attachment.url} target="_blank" rel="noreferrer"><img src={attachment.url} alt={attachment.title}/></a> : <a className="attachment-file" href={attachment.url} target="_blank" rel="noreferrer">{link?<Link2 size={22}/>:<File size={22}/>}</a>}<div><a href={attachment.url} target="_blank" rel="noreferrer">{attachment.title}</a><small>{link?new URL(attachment.url).hostname:formatBytes(attachment.size)}</small></div><button type="button" aria-label={`Delete attachment ${attachment.title}`} onClick={() => onDelete(attachment.id)}><Trash2 size={13}/></button></article> })}
