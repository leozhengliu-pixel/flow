...
function SubIssueMoreMenu({ data, dueDate, project, onDueDate, onProject }: { data: BootstrapData; dueDate: string; project?: BootstrapData['projects'][number]; onDueDate: (value: string) => void; onProject: (id: string) => void }) {
  return <DropdownMenu.Root><DropdownMenu.Trigger asChild><button className="sub-issue-more" type="button" role="combobox" aria-label="More actions"><MoreHorizontal/></button></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className="sub-issue-more-menu" sideOffset={4} align="start">
    <DropdownMenu.Sub><DropdownMenu.SubTrigger><ProjectIcon/><span>Project</span><kbd>⇧ P</kbd><ChevronRight/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent data-flow-motion="floating" className="sub-issue-more-menu" sideOffset={4}><DropdownMenu.Item onSelect={() => onProject('')}><NoProjectIcon/><span>No project</span></DropdownMenu.Item>{data.projects.map(item => <DropdownMenu.Item key={item.id} onSelect={() => onProject(item.id)}><ProjectIcon style={{ color: item.color }}/><span data-i18n-ignore>{item.name}</span>{item.id === project?.id && <span className="sub-issue-selected">✓</span>}</DropdownMenu.Item>)}</DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub>
    <DropdownMenu.Sub><DropdownMenu.SubTrigger><CalendarDays/><span>{dueDate || 'Set due date'}</span><kbd>D</kbd><ChevronRight/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent data-flow-motion="floating" className="sub-issue-due-menu" sideOffset={4}><DueDateCommand value={dueDate} onSelect={async value => onDueDate(value)}/></DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub>
