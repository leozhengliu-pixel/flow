...
function OperatorMenu<TOption extends AppliedFilterOption>({ filter, onChange, translate }: { filter: AppliedFilterItem<TOption>; onChange: (operator: AppliedFilterOperator) => void; translate: (value: string) => string }) {
  ...
  return <Popover.Root><Popover.Trigger asChild><button className={styles.operator} type="button" aria-label={`${filter.fieldLabel} operator`}>{filter.operator === 'is' ? positive : negative}</button></Popover.Trigger><Popover.Portal><Popover.Content data-flow-motion="floating" align="start" className={styles.operatorMenu} collisionPadding={8} sideOffset={4}>
