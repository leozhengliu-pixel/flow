  ...
  if (mode === 'invite') {
    return <AuthShell>
      ...
      {pending && !invitation ? <LoaderCircle className="auth-spinner"/> : invitation && <>
        ...
        {session && session.user.email.toLowerCase() !== invitation.email.toLowerCase() && <div className="auth-error" role="alert">Sign in as {invitation.email} to accept this invitation.</div>}
      </>}
