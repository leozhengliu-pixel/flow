...
function ReleaseList({data,pipeline,releases,archived,onCreate,onDelete,onEdit,onOpen,onReload}:{data:BootstrapData;pipeline:ReleasePipeline;releases:Release[];archived:boolean;onCreate:()=>void;onDelete:(release:Release)=>void;onEdit:(release:Release)=>void;onOpen:(release:Release)=>void;onReload:()=>Promise<void>}) {
  ...
  if(!releases.length)return <ReleaseEmptyState icon={archived?<Archive/>:<Rocket/>} title={t(archived?'No archived releases':pipeline.type==='continuous'?'Awaiting first release':'Create your first release')} description={t(archived?'Archived releases will appear here.':pipeline.type==='continuous'?'Releases added to this pipeline will appear here. Integrate with your CI/CD tool to automatically group issues into releases.':"Create a new release to start tracking what's shipping.")} action={!archived&&pipeline.type==='scheduled'?<button className="flow-releases-primary" onClick={onCreate}>{t('Create new release')}</button>:pipeline.type==='continuous'?<a className="flow-releases-primary flow-releases-doc-link" href="https://linear.app/docs/releases#example-for-continuous-deployments" target="_blank" rel="noreferrer">{t('Set up integration')}</a>:undefined}/>
