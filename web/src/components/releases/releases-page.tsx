...
import { Archive, ArchiveRestore, ArrowDownWideNarrow, ArrowUpNarrowWide, Check, ChevronDown, ChevronRight, CircleDashed, Copy, FileClock, FileText, Link2, Menu, MoreHorizontal, PanelRightClose, PanelRightOpen, Pencil, Plus, Rocket, Search, Settings2, Star, Trash2, X } from 'lucide-react'
...
import { createReleaseNote, deleteRelease, recordRecentResource, updateIssue, updateRelease, updateReleaseNote } from '@/lib/api'
...
function ReleaseRouteNotFound({title,onOpenSidebar}:{title:string;onOpenSidebar:()=>void}){return <main className="main-panel flow-releases-page"><TopBar title={<h1>{title}</h1>} onOpenSidebar={onOpenSidebar}/><ReleaseEmptyState icon={<Rocket/>} title={title} description="404"/></main>}
...
function ReleaseList({data,pipeline,releases,archived,onCreate,onDelete,onEdit,onOpen,onReload}:{data:BootstrapData;pipeline:ReleasePipeline;releases:Release[];archived:boolean;onCreate:()=>void;onDelete:(release:Release)=>void;onEdit:(release:Release)=>void;onOpen:(release:Release)=>void;onReload:()=>Promise<void>}) {
  ...
  if(!releases.length)return <ReleaseEmptyState icon={archived?<Archive/>:<Rocket/>} title={t(archived?'No archived releases':pipeline.type==='continuous'?'Awaiting first release':'Create your first release')} description={t(archived?'Archived releases will appear here.':pipeline.type==='continuous'?'Releases added to this pipeline will appear here. Integrate with your CI/CD tool to automatically group issues into releases.':"Create a new release to start tracking what's shipping.")} action={!archived&&pipeline.type==='scheduled'?<button className="flow-releases-primary" onClick={onCreate}>{t('Create new release')}</button>:pipeline.type==='continuous'?<a className="flow-releases-primary flow-releases-doc-link" href="https://flow.app/docs/releases#continuous-deployments" target="_blank" rel="noreferrer">{t('Set up integration')}</a>:undefined}/>
