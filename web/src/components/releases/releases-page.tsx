...
import { CalendarIcon, TeamIcon } from '@/components/issue/issue-icons'
...
function ReleaseRouteNotFound({title,onOpenSidebar}:{title:string;onOpenSidebar:()=>void}){return <main className="main-panel flow-releases-page"><TopBar title={<h1>{title}</h1>} onOpenSidebar={onOpenSidebar}/><ReleaseEmptyState icon={<ReleasesIcon/>} title={title} description="404"/></main>}
...
function ReleasePipelinesView({ data, onCreate, onOpen, onOpenSidebar, onNavigate }: { data: BootstrapData; onCreate: () => void; onOpen: (pipeline: ReleasePipeline) => void; onOpenSidebar: () => void; onNavigate:(path:string)=>void }) {
  ...
  const releasesOptions = <DropdownMenu.Root open={optionsOpen} onOpenChange={open => { setOptionsOpen(open); if (!open) setOptionsQuery('') }}><DropdownMenu.Trigger asChild><IconButton className="flow-releases-directory-options" label={t('Releases options')}><MoreHorizontal/></IconButton></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className="flow-releases-menu flow-releases-directory-menu" align="start" sideOffset={5}><label className="flow-releases-directory-menu__search"><Search/><input ref={optionsSearchRef} aria-label={t('Filter…')} onChange={event => setOptionsQuery(event.target.value)} onKeyDown={event => { if (event.key !== 'Escape') event.stopPropagation() }} placeholder={t('Filter…')} role="searchbox" value={optionsQuery}/></label>{showSettings ? <ReleaseMenuItem icon={<Settings2/>} onSelect={() => onNavigate(`/${data.workspace.urlKey}/settings/releases`)}>{settingsLabel}</ReleaseMenuItem> : <div className="flow-releases-directory-menu__empty">{t('No results')}</div>}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
  return <main className="main-panel flow-releases-page" aria-label={t('Releases')}>
    ...
    <div className="flow-pipelines-summary"><ContentViewHeaderSearch alwaysVisible placeholder={t('Find release pipelines…')} value={query} onChange={setQuery} className="flow-pipeline-search-cvh"/><Popover.Root open={displayOpen} onOpenChange={setDisplayOpen}><Popover.Trigger asChild><button aria-label={t('Display options')} aria-pressed={displayOpen}><SlidersHorizontal/></button></Popover.Trigger><Popover.Portal><Popover.Content data-flow-motion="floating" aria-label={t('Display options')} className="flow-pipeline-display-popover" align="end" collisionPadding={8} sideOffset={4}>
...
function ReleaseDetailPage({data,pipeline,release,tab,onNavigate,onOpenSidebar,onDelete,onEdit,onReload}:{data:BootstrapData;pipeline:ReleasePipeline;release:Release;tab:ReleaseRouteTab;onNavigate:(path:string)=>void;onOpenSidebar:()=>void;onDelete:()=>void;onEdit:()=>void;onReload:()=>Promise<void>}) {
  ...
  const [issuesLoading,setIssuesLoading]=useState(()=>release.issueIds.length>0)
  const loadedIssueScope=useRef('')
  const associatedIssueKey=release.issueIds.join(',')
  useEffect(()=>{
    const issueScope=`${release.id}:${associatedIssueKey}`
    if(loadedIssueScope.current!==issueScope){
      loadedIssueScope.current=issueScope
      setLoadedIssues([])
    }
    if(!associatedIssueKey){setLoadedIssues([]);setIssuesLoading(false);return}
    ...
    const load=async()=>{
      ...
      try {
        ...
        if(!cancelled) setLoadedIssues(collected)
      } catch (error) {
  ...
  },[associatedIssueKey,data.issueCollectionRevision,release.id,t])
  const issues=loadedIssues
  ...
  const toggleFavorite=()=>{void toggleFavoriteFor(data,'release',release.id,undefined,favorite)}
  const copyUrl=async()=>{try{await navigator.clipboard.writeText(window.location.href);toast.success(t('URL copied'))}catch{toast.error(t('Could not copy URL'))}}
  ...
  const releaseMenu=<DropdownMenu.Root onOpenChange={open=>{if(!open)setReleaseMenuQuery('')}}><DropdownMenu.Trigger asChild><IconButton label={t('Release options')}><MoreHorizontal/></IconButton></DropdownMenu.Trigger><DropdownMenu.Portal><DropdownMenu.Content data-flow-motion="floating" className="flow-releases-menu flow-release-options" align="start" sideOffset={4}><input className="flow-release-options__filter" aria-label={t('Filter…')} autoFocus onChange={event=>setReleaseMenuQuery(event.target.value)} onKeyDown={event=>{if(event.key!=='Escape')event.stopPropagation()}} placeholder={t('Filter…')} role="searchbox" value={releaseMenuQuery}/>{menuMatches('Edit…')&&<ReleaseMenuItem icon={<Pencil/>} onSelect={onEdit}>{t('Edit…')}</ReleaseMenuItem>}{menuMatches('Stage')&&<DropdownMenu.Sub><DropdownMenu.SubTrigger className="flow-release-menu-item"><ReleaseStageIcon fallback={release.status} pipeline={pipeline} stage={stage}/><span>{t('Stage')}</span><ChevronRight/></DropdownMenu.SubTrigger><DropdownMenu.Portal><DropdownMenu.SubContent data-flow-motion="floating" className="flow-releases-menu flow-release-stage-menu" sideOffset={4}>{pipeline.stages.map(value=><ReleaseStageMenuItem key={value} onSelect={()=>void updateStage(value)} pipeline={pipeline} selected={value===stage} value={value}/>)}</DropdownMenu.SubContent></DropdownMenu.Portal></DropdownMenu.Sub>}<DropdownMenu.Separator/>{menuMatches('Add issues to release…')&&<ReleaseMenuItem icon={<Plus/>} shortcut="⌥ R" onSelect={onEdit}>{t('Add issues to release…')}</ReleaseMenuItem>}{menuMatches('Add document…')&&<ReleaseMenuItem icon={<FileText/>} onSelect={()=>setDocumentOpen(true)}>{t('Add document…')}</ReleaseMenuItem>}{menuMatches('Add link…')&&<ReleaseMenuItem icon={<Link2/>} shortcut="Ctrl L" onSelect={()=>setLinkOpen(true)}>{t('Add link…')}</ReleaseMenuItem>}<DropdownMenu.Separator/>{menuMatches(favorite?'Remove from favorites':'Favorite')&&<ReleaseMenuItem icon={<Star fill={favorite?'currentColor':'none'}/>} shortcut="⌥ F" onSelect={()=>void toggleFavorite()}>{t(favorite?'Remove from favorites':'Favorite')}</ReleaseMenuItem>}{menuMatches('Copy URL')&&<ReleaseMenuItem icon={<Copy/>} shortcut="⌘ ⇧ ," onSelect={()=>void copyUrl()}>{t('Copy URL')}</ReleaseMenuItem>}<DropdownMenu.Separator/>{menuMatches('Delete')&&<ReleaseMenuItem danger icon={<Trash2/>} onSelect={onDelete}>{t('Delete')}</ReleaseMenuItem>}</DropdownMenu.Content></DropdownMenu.Portal></DropdownMenu.Root>
...
function BasicDialog({title,onClose,children}:{title:string;onClose:()=>void;children:ReactNode}){const{t}=useI18n();return <Dialog.Root open onOpenChange={open=>!open&&onClose()}><Dialog.Portal><Dialog.Overlay data-flow-motion="backdrop" className="flow-release-dialog-overlay"/><Dialog.Content data-flow-motion="dialog" aria-describedby={undefined} className="flow-release-basic-dialog"><Dialog.Title>{title}</Dialog.Title><Dialog.Close aria-label={t('Close')}><X/></Dialog.Close>{children}</Dialog.Content></Dialog.Portal></Dialog.Root>}
const IconButton = forwardRef<HTMLButtonElement, {label:string;children:ReactNode} & Omit<ComponentPropsWithoutRef<'button'>, 'aria-label'|'children'>>(({label,children,className,...props},ref)=><button {...props} ref={ref} className={`flow-releases-icon-button ui-pill${className?` ${className}`:''}`} aria-label={label} title={label}>{children}</button>)
IconButton.displayName='IconButton'
function ReleaseMenuItem({icon,children,onSelect,danger,shortcut}:{icon:ReactNode;children:ReactNode;onSelect:()=>void;danger?:boolean;shortcut?:string}){return <DropdownMenu.Item className={`flow-release-menu-item${danger?' danger':''}`} onSelect={onSelect}>{icon}<span>{children}</span>{shortcut&&<kbd>{shortcut}</kbd>}</DropdownMenu.Item>}
function ReleaseStageIcon({pipeline,stage,fallback,...props}:{pipeline:ReleasePipeline;stage?:string;fallback?:Release['status']} & Omit<ComponentPropsWithoutRef<typeof ReleaseStatusIcon>,'status'>) {
  return <ReleaseStatusIcon status={releaseStatusForStage(pipeline, stage || '', fallback)} {...props}/>
}
function ReleaseStageMenuItem({pipeline,value,selected,onSelect}:{pipeline:ReleasePipeline;value:string;selected:boolean;onSelect:()=>void}) {
  return <DropdownMenu.Item className="flow-release-menu-item" onSelect={onSelect}><ReleaseStageIcon pipeline={pipeline} stage={value}/><span data-i18n-ignore>{value}</span>{selected&&<Check/>}</DropdownMenu.Item>
}
function ReleaseEmptyState({icon,title,description,action}:{icon:ReactNode;title:ReactNode;description:ReactNode;action?:ReactNode}){return <div className="flow-release-empty">{icon}<strong>{title}</strong><span>{description}</span>{action&&<div>{action}</div>}</div>}
