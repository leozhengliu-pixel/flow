...
function OperationsTopBar({ title, onOpenSidebar, children }: { title: ReactNode; onOpenSidebar: () => void; children?: ReactNode }) {
  return <header className="operations-header operations-special-header"><button className="operations-mobile-menu" aria-label="Open sidebar" data-sidebar-trigger onClick={onOpenSidebar}><Menu/></button><div className="operations-heading">{title}</div><div className="operations-header-actions">{children}</div></header>
}
...
function AsksPage({ data, surface, onSurfaceChange, onOpenSidebar, onCreate, onOpenSettings, onDecide, onDelete }: { data: BootstrapData; surface: 'configuration'|'intake'; onSurfaceChange: (value: 'configuration'|'intake') => void; onOpenSidebar: () => void; onCreate: () => void; onOpenSettings:(page:'asks'|'integrations')=>void; onDecide: (ask: Ask, decision: 'approved'|'rejected') => void; onDelete: (id: string) => Promise<void> }) {
  ...
  const slack=data.integrationConnections.find(item=>item.provider==='slack'&&item.status==='connected')
  const emails=data.workspaceSettings.featureSettings?.asksEmailAddresses??[]
  const mapped=data.workspaceSettings.featureSettings?.asksSlackChannels?.length??0
  return <main className="main-panel operations-page asks-page" aria-label="Asks">
    ...
    <div className="asks-content">
      ...
      {surface === 'configuration' ? <div className="asks-config">
        <section><header><h3>Slack</h3><p>Allow anyone in your Slack workspace to submit Asks using templated forms</p></header><button className="asks-config-row" onClick={()=>onOpenSettings('asks')}><span className="asks-source-icon slack"><MessageCircleQuestion/></span><span><strong data-i18n-ignore={slack?true:undefined}>{slack?slack.name:'No workspaces connected'}</strong><small>{slack?(mapped?`${mapped} channels mapped`:'Connected · configure channels in Asks settings'):'Connect Slack to route requests into team Triage.'}</small></span><Plus/></button></section>
        <section><header><h3>Email</h3><p>Allow anyone to submit Asks by emailing a custom address</p></header><button className="asks-config-row" onClick={()=>onOpenSettings('asks')}><span className="asks-source-icon mail"><Mail/></span><span><strong data-i18n-ignore={emails.length?true:undefined}>{emails.length?emails[0]:'No email addresses configured'}</strong><small>{emails.length?`${emails.length} address${emails.length===1?'':'es'} configured`:'Create an intake address and choose a team and template.'}</small></span><Plus/></button></section>
...
function DraftsPage({ data, onOpenSidebar, onReload, onNavigate, onResume }: { data: BootstrapData; onOpenSidebar: () => void; onReload: () => Promise<void>; onNavigate: (path: string) => void; onResume: (draft: Draft) => void }) {
  ...
  return <main className="main-panel operations-page drafts-page" aria-label={t('Drafts')}>
    <header className="operations-header drafts-header"><button className="operations-mobile-menu" aria-label={t('Open sidebar')} data-sidebar-trigger onClick={onOpenSidebar}><Menu/></button><h2>{t('Drafts')}</h2>{drafts.length > 0 && <button className="draft-discard-all" aria-label={t('Discard all')} onClick={() => setConfirm({ kind: 'all' })} type="button"><Trash2/></button>}</header>
