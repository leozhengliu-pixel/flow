...
import { Archive, ArchiveRestore, Check, ChevronDown, ChevronRight, CircleDashed, Clock3, FileClock, FilePenLine, Filter, History, Inbox, Mail, Menu, MessageCircleQuestion, MoreHorizontal, Plus, Rocket, Search, Settings2, SlidersHorizontal, Star, Trash2, X } from 'lucide-react'
...
function CreateOperationDialog({ view, data, release, onClose, onCreated }: { view: 'releases'|'asks'; data: BootstrapData; release?: Release; onClose: () => void; onCreated: () => Promise<void> }) {
  ...
  if(view==='releases') return <Dialog open onOpenChange={open=>!open&&!saving&&onClose()}><DialogContent aria-describedby={undefined} className={`release-editor-dialog${scopeOpen?' is-scope-open':''}`}>
    ...
    <div className="release-editor-properties">
      ...
      <label className="release-editor-pill release-date-pill"><CalendarIcon/><span className={targetDate?'has-value':''}>{targetDate||'Target date'}</span><input aria-label="Target date" type="date" value={targetDate} onChange={event=>setTargetDate(event.target.value)}/></label>
