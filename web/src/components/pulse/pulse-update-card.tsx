...
export function PulseUpdateCard(props: Props) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      if (item.kind === 'project') await props.onUpdateProject(item.project.id, update.id, { body: editBody.trim(), health: editHealth })
      else await props.onUpdateInitiative(item.initiative.id, update.id, { body: editBody.trim(), health: editHealth })
      setEditing(false)
