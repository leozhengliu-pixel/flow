...
function describe(event:ActivityEvent,t:(source:string)=>string){if(event.type==='issue.created')return'created the issue';if(event.type==='issue.updated'){const fields=Object.keys(event.metadata).filter(field=>!['documentContent'].includes(field));return fields.length?`changed ${fields.join(', ')}`:'updated the issue'};if(event.type==='issue.releases_updated'){if(event.metadata.added)return`${t('added to release')} ${event.metadata.added}`;if(event.metadata.removed)return`${t('removed from release')} ${event.metadata.removed}`;return t('updated releases')};if(event.type==='issue.review_linked')return`linked pull request ${event.metadata.repository||''}${event.metadata.reviewNumber?` #${event.metadata.reviewNumber}`:''}`;if(event.type==='issue.review_unlinked')return`unlinked pull request ${event.metadata.repository||''}${event.metadata.reviewNumber?` #${event.metadata.reviewNumber}`:''}`;if(event.type==='issue.relation_added')return`marked this as ${relationText(event.metadata.type)} ${event.metadata.relatedIssueId||''}`;if(event.type==='issue.relation_removed')return'removed an issue relation';if(event.type==='attachment.created')return`attached ${event.metadata.title||'a file'}`;if(event.type==='attachment.deleted')return'removed an attachment';return event.type.replaceAll('.',' ').replaceAll('_',' ')}
