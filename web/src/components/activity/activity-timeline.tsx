...
function ActivityRow({event}:{event:ActivityEvent}) { return <p className="activity-row"><span><strong>{event.actor.displayName}</strong> {describe(event)}</span><span className="activity-dot">·</span><a href={`#activity-${event.id}`} title={format(new Date(event.createdAt),'PPpp')}>{formatDistanceToNow(new Date(event.createdAt),{addSuffix:true})}</a></p> }
...
function describe(event:ActivityEvent){if(event.type==='issue.created')return'created the issue';if(event.type==='issue.updated'){const fields=Object.keys(event.metadata).filter(field=>!['documentContent'].includes(field));return fields.length?`changed ${fields.join(', ')}`:'updated the issue'};if(event.type==='issue.relation_added')return`marked this as ${relationText(event.metadata.type)} ${event.metadata.relatedIssueId||''}`;if(event.type==='issue.relation_removed')return'removed an issue relation';if(event.type==='attachment.created')return`attached ${event.metadata.title||'a file'}`;if(event.type==='attachment.deleted')return'removed an attachment';return event.type.replaceAll('.',' ').replaceAll('_',' ')}
