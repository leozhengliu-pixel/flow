...
import { fetchIssueHistory, updateComment } from '@/lib/api'
import { PagedActivityTimeline } from './paged-activity-timeline'

vi.mock('@/lib/api', () => ({fetchIssueHistory:vi.fn(),updateComment:vi.fn(),createComment:vi.fn(),deleteComment:vi.fn(),toggleCommentReaction:vi.fn()}))
...
it('loads older pages without accumulating history and edits the displayed page', async () => {
  ...
  vi.mocked(fetchIssueHistory).mockResolvedValueOnce({comments:[old],activities:[]})
    .mockResolvedValueOnce({comments:[recent],activities:[],commentsCursor:'older'})
  ...
  expect(fetchIssueHistory).toHaveBeenCalledWith('issue-1',expect.any(AbortSignal),{commentsCursor:'older',activitiesCursor:undefined})
