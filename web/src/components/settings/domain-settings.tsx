...
function GroupRow({ group, collapsed = false, selected, resourceType, usage = 0, lastApplied, onToggleCollapsed, onToggleSelected, onCreateLabel, onSave, onArchive, onDelete }: { group: LabelGroup; collapsed?: boolean; selected: boolean; resourceType: 'issue'|'project'; usage?: number; lastApplied?: string; onToggleCollapsed?: () => void; onToggleSelected: () => void; onCreateLabel?: () => void; onSave?: (input: Partial<LabelGroup>) => Promise<void>; onArchive: () => Promise<void>; onDelete: () => Promise<void> }) {
...
function LabelConfirmDialog({kind,action,name,archived,resourceType,usage,onArchive,onClose,onConfirm}:{kind:'label'|'group';action:'archive'|'delete'|null;name:string;archived:boolean;resourceType:'issue'|'project';usage:number;onArchive:()=>void|Promise<void>;onClose:()=>void;onConfirm:()=>Promise<void>}){
...
function labelUsage(label: IssueLabel, data: BootstrapData, resourceType: 'issue'|'project') { return label.issueCount ?? (resourceType === 'issue' ? data.issues.filter(issue => !issue.archivedAt && issue.labels.some(item => item.id === label.id)).length : data.projects.filter(project => !project.archivedAt && (project.labelIds ?? []).includes(label.id)).length) }
