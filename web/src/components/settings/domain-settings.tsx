  ...
  return <div className="domain-labels-page" data-i18n-ignore>
    ...
    <section className="settings-section domain-labels-section"><div className="domain-labels-grid">
      <div className="domain-labels-table-header">
        ...
        <LabelSortHeader label={t(resourceType === 'issue' ? 'Issues' : resourceType === 'project' ? 'Projects' : 'Initiatives')} sortKey="usage" sort={sort} onSort={setSort}/>
...
function ScopeButton({ value, resourceType, onChange }: { value: ScopeFilter; resourceType: LabelResourceType; onChange: (value: ScopeFilter) => void }) {
  const { t } = useI18n()
...
function LabelSectionRows({ section, data, resourceType, availableGroups, selected, creatingGroupId, onCancelCreate, onCreateInGroup, onCreateLabel, onToggleGroupSelected, onToggleSelected, onSaveLabel, onArchiveLabel, onMoveLabelToTeams, onDeleteLabel, onSaveGroup, onArchiveGroup, onDeleteGroup }: {
  section: LabelSection; data: BootstrapData; resourceType: LabelResourceType; availableGroups: LabelGroup[]; selected: string[]; creatingGroupId?: string;
  ...
  return <>
    {section.group && <GroupRow group={section.group} collapsed={collapsed} selected={selected.includes(section.group.id)} resourceType={resourceType} usage={section.labels.reduce((sum,label)=>sum+labelUsage(label,data,resourceType),0)} lastApplied={section.labels.map(label=>label.lastAppliedAt??'').sort().at(-1)} onToggleCollapsed={() => setCollapsed(value => !value)} onToggleSelected={() => onToggleGroupSelected(section.group!, section.labels.map(label => label.id))} onCreateLabel={() => onCreateInGroup(section.group!.id)} onSave={input => onSaveGroup(section.group!, input)} onArchive={() => onArchiveGroup(section.group!)} onDelete={() => onDeleteGroup(section.group!)}/>}
...
function GroupRow({ group, collapsed = false, selected, resourceType, usage = 0, lastApplied, onToggleCollapsed, onToggleSelected, onCreateLabel, onSave, onArchive, onDelete }: { group: LabelGroup; collapsed?: boolean; selected: boolean; resourceType: LabelResourceType; usage?: number; lastApplied?: string; onToggleCollapsed?: () => void; onToggleSelected: () => void; onCreateLabel?: () => void; onSave?: (input: Partial<LabelGroup>) => Promise<void>; onArchive: () => Promise<void>; onDelete: () => Promise<void> }) {
...
function LabelRowMenu({ label, groups, workspaceSlug, resourceType, onEdit, onGroup, onArchive, onMoveToTeams, onDelete }: { label: IssueLabel; groups: LabelGroup[]; workspaceSlug: string; resourceType:LabelResourceType; onEdit: () => void; onGroup: (groupId: string) => Promise<void>; onArchive: () => void; onMoveToTeams?: () => Promise<void>; onDelete: () => Promise<void> }) {
  const { locale, t } = useI18n()
  const [moveOpen, setMoveOpen] = useState(false)
  const [moving, setMoving] = useState(false)
  const labeledIssuesHref = `/${encodeURIComponent(workspaceSlug)}/${resourceType==='project'?'projects/all':'issues/all'}?label=${encodeURIComponent(label.id)}`
  ...
  return <><MenuShell kind="label" label={label.name}>{label.archivedAt ? <><DropdownMenuItem onSelect={onArchive}><MenuIcon name="restore"/><span>{t('Restore')}</span></DropdownMenuItem><DropdownMenuItem className="danger-item" onSelect={() => void onDelete()}><MenuIcon name="delete"/><span>{t('Delete')}</span></DropdownMenuItem></> : <><DropdownMenuItem onSelect={onEdit}><MenuIcon name="edit"/><span>{t('Edit label name')}</span><MenuShortcut>E</MenuShortcut></DropdownMenuItem>{canMoveToGroup && <DropdownMenuSub><DropdownMenuSubTrigger><MenuIcon name="move"/><span>{t('Move to group')}</span></DropdownMenuSubTrigger><DropdownMenuSubContent data-i18n-ignore className="domain-label-group-submenu" sideOffset={4}>{label.groupId && <DropdownMenuItem onSelect={() => void onGroup('')}><span>{t('Remove from group')}</span></DropdownMenuItem>}{label.groupId && groupOptions.length > 0 && <DropdownMenuSeparator/>}{groupOptions.map(group => <DropdownMenuItem key={group.id} onSelect={() => void onGroup(group.id)}><PaletteMark color={group.color}/><span>{group.name}</span></DropdownMenuItem>)}</DropdownMenuSubContent></DropdownMenuSub>}{onMoveToTeams && <DropdownMenuItem onSelect={() => setMoveOpen(true)}><MenuIcon name="teams"/><span>{t('Move to teams…')}</span></DropdownMenuItem>}<DropdownMenuSeparator/><DropdownMenuItem asChild><AppLink href={labeledIssuesHref}><MenuIcon name="view"/><span>{t(resourceType==='project'?'View labeled projects':'View labeled issues')}</span></AppLink></DropdownMenuItem><DropdownMenuSeparator/><DropdownMenuItem onSelect={onArchive}><MenuIcon name="archive"/><span>{t('Archive…')}</span></DropdownMenuItem><DropdownMenuItem className="danger-item" onSelect={() => void onDelete()}><MenuIcon name="delete"/><span>{t('Delete')}</span></DropdownMenuItem></>}</MenuShell><Dialog open={moveOpen} onOpenChange={setMoveOpen}><DialogContent data-i18n-ignore className="domain-label-move-dialog"><DialogTitle>{locale === 'zh-CN' ? <>将“<strong>{label.name}</strong>”移动到团队？</> : <>Move <strong>"{label.name}"</strong> to teams?</>}</DialogTitle><p>{t('This label will be moved to each team that uses it and will no longer be available for the whole workspace.')}</p><p>{t('This action cannot be undone.')}</p><footer><button disabled={moving} onClick={() => setMoveOpen(false)}>{t('Cancel')}</button><button className="primary" disabled={moving} onClick={() => void move()}>{t(moving ? 'Moving…' : 'Move label')}</button></footer></DialogContent></Dialog></>
}
...
function LabelConfirmDialog({kind,action,name,archived,resourceType,usage,onArchive,onClose,onConfirm}:{kind:'label'|'group';action:'archive'|'delete'|null;name:string;archived:boolean;resourceType:LabelResourceType;usage:number;onArchive:()=>void|Promise<void>;onClose:()=>void;onConfirm:()=>Promise<void>}){
  ...
  const noun=resourceType==='issue'?'issue':'project'
  const plural=usage===1?noun:`${noun}s`
  ...
  const archiveCopy=locale==='zh-CN'
    ? kind==='group'?`此分组内的标签将无法再应用到${resourceType==='issue'?'事项':'项目'}。已经应用这些标签的${resourceType==='issue'?'事项':'项目'}不会改变。`:`此标签将无法再应用到${resourceType==='issue'?'事项':'项目'}。已经应用此标签的${resourceType==='issue'?'事项':'项目'}不会改变。`
    : kind==='group'?`The labels in this group will no longer be available to apply to ${noun}s. ${noun[0].toUpperCase()+noun.slice(1)}s with these labels already applied will remain unchanged.`:`This label will no longer be available to apply to ${noun}s. ${noun[0].toUpperCase()+noun.slice(1)}s with this label already applied will remain unchanged.`
...
function ArchivedRows({ groups, labels, data, resourceType, availableGroups, selected, onToggleGroupSelected, onToggleSelected, onSaveLabel, onArchiveLabel, onDeleteLabel, onArchiveGroup, onDeleteGroup }: { groups: LabelGroup[]; labels: IssueLabel[]; data: BootstrapData; resourceType: LabelResourceType; availableGroups: LabelGroup[]; selected: string[]; onToggleGroupSelected: (group: LabelGroup, childIds: string[]) => void; onToggleSelected: (id: string) => void; onSaveLabel: (label: IssueLabel, input: Partial<IssueLabel>) => Promise<void>; onArchiveLabel: (label: IssueLabel) => void; onDeleteLabel: (label: IssueLabel) => Promise<void>; onArchiveGroup: (group: LabelGroup) => Promise<void>; onDeleteGroup: (group: LabelGroup) => Promise<void> }) {
  const { t } = useI18n()
  const childIds = (group: LabelGroup) => labels.filter(label => label.groupId === group.id).map(label => label.id)
  return <><div className="domain-label-scope-header is-static"><strong>{t('Workspace')}</strong><span>{groups.length + labels.length}</span></div>{[...groups].sort((a, b) => a.name.localeCompare(b.name)).map(group => {const children=labels.filter(label=>label.groupId===group.id);return <GroupRow key={group.id} group={group} selected={selected.includes(group.id)} resourceType={resourceType} usage={children.reduce((sum,label)=>sum+labelUsage(label,data,resourceType),0)} lastApplied={children.map(label=>label.lastAppliedAt??'').sort().at(-1)} onToggleSelected={() => onToggleGroupSelected(group, childIds(group))} onArchive={() => onArchiveGroup(group)} onDelete={() => onDeleteGroup(group)}/>})}{labels.map(label => <LabelRow key={label.id} data={data} label={label} grouped={false} resourceType={resourceType} selected={selected.includes(label.id)} groups={availableGroups} onToggleSelected={() => onToggleSelected(label.id)} onSave={input => onSaveLabel(label, input)} onArchive={() => onArchiveLabel(label)} onDelete={() => onDeleteLabel(label)}/>)}</>
}
