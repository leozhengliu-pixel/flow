  ...
  return <div className="domain-labels-page" data-i18n-ignore>
    ...
    <section className="settings-section domain-labels-section"><div className="domain-labels-grid">
      <div className="domain-labels-table-header">
        <SortHeader label={t('Name')} sortKey="name" sort={sort} onSort={setSort}/>
        <SortHeader label={t('Description')} sortKey="description" sort={sort} onSort={setSort}/>
        <SortHeader label={t(resourceType === 'issue' ? 'Issues' : 'Projects')} sortKey="usage" sort={sort} onSort={setSort}/>
        <SortHeader label={t('Last applied')} sortKey="lastAppliedAt" sort={sort} onSort={setSort}/>
        <SortHeader label={t(scope === 'archived' ? 'Archived' : 'Created')} sortKey="createdAt" sort={sort} onSort={setSort}/>
      </div>
      {creating?.kind === 'group' && (
...
function SortHeader({ label, sortKey, sort, onSort }: { label: string; sortKey: LabelSort; sort: { key: LabelSort; descending: boolean }; onSort: (value: { key: LabelSort; descending: boolean }) => void }) {
...
function LabelSectionRows({ section, data, resourceType, availableGroups, selected, creatingGroupId, onCancelCreate, onCreateInGroup, onCreateLabel, onToggleGroupSelected, onToggleSelected, onSaveLabel, onArchiveLabel, onMoveLabelToTeams, onDeleteLabel, onSaveGroup, onArchiveGroup, onDeleteGroup }: {
  ...
  return <>
    ...
    {!collapsed && section.group && creatingGroupId === section.group.id && <InlineLabelRow kind="label" grouped continuous onCancel={onCancelCreate} onSave={input => onCreateLabel(input, section.group!.id)}/>}
    {!collapsed && section.labels.map(label => <LabelRow key={label.id} data={data} label={label} grouped={Boolean(section.group)} resourceType={resourceType} selected={selected.includes(label.id)} groups={availableGroups} onToggleSelected={() => onToggleSelected(label.id)} onSave={input => onSaveLabel(label, input)} onArchive={() => onArchiveLabel(label)} onMoveToTeams={() => onMoveLabelToTeams(label)} onDelete={() => onDeleteLabel(label)}/>)}</>
}

interface NewLabelInput { name: string; description: string; color: string }
function InlineLabelRow({ kind, grouped = false, continuous = false, onCancel, onSave }: { kind: 'label'|'group'; grouped?: boolean; continuous?: boolean; onCancel: () => void; onSave: (input: NewLabelInput) => Promise<boolean> }) {
  const { t } = useI18n()
...
function EditableText({ value, field, editing, archived, onEdit, onCancel, onSave }: { value: string; field: string; editing: boolean; archived: boolean; onEdit: () => void; onCancel: () => void; onSave: (value: string) => Promise<void> }) {
  const [draft, setDraft] = useState(value)
  if (editing) return <input autoFocus className="domain-label-inline-input" aria-label={`Edit ${field}`} value={draft} onChange={event => setDraft(event.target.value)} onBlur={() => { const next = draft.trim(); if (next && next !== value) void onSave(next); else onCancel() }} onKeyDown={event => { if (event.key === 'Enter') event.currentTarget.blur(); if (event.key === 'Escape') { setDraft(value); onCancel() } }}/>
  return <button className="domain-label-text" disabled={archived} onClick={onEdit}>{value}</button>
}
...
function LabelRowMenu({ label, groups, workspaceSlug, onEdit, onGroup, onArchive, onMoveToTeams, onDelete }: { label: IssueLabel; groups: LabelGroup[]; workspaceSlug: string; onEdit: () => void; onGroup: (groupId: string) => Promise<void>; onArchive: () => void; onMoveToTeams?: () => Promise<void>; onDelete: () => Promise<void> }) {
  const { locale, t } = useI18n()
  const [moveOpen, setMoveOpen] = useState(false)
  const [moving, setMoving] = useState(false)
  const viewLabeledIssues = () => { window.location.href = `/${encodeURIComponent(workspaceSlug)}/issues/all?label=${encodeURIComponent(label.id)}` }
