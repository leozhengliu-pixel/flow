...
function PersonalPageTitle({ children, description }: { children: ReactNode; description?: ReactNode }) { return <SettingsPageTitle className="personal-page-header" description={description}>{children}</SettingsPageTitle> }
function PersonalSection({ title, description, children }: { title?: string; description?: string; children: ReactNode }) { return <SettingsSection className="personal-section" description={description} title={title}>{children}</SettingsSection> }
function PersonalRow(props: React.ComponentProps<typeof SettingsRow>) { return <SettingsRow {...props}/> }
function PersonalSelect({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <SettingsSelect className="personal-select" entityName={isBusinessName} label={label} menuClassName="personal-select-menu" onChange={onChange} options={options} value={value}/> }
...
function Preferences({ values, setValue, text }: PersonalProps) {
  ...
  return <><PersonalPageTitle>{text.preferences}</PersonalPageTitle>
    <PersonalSection title={text.general}>
      <PersonalRow title={text.language} description={text.languageDescription}><PersonalSelect label={text.language} value={locale === "zh-CN" ? "简体中文" : "English"} options={["English", "简体中文"]} onChange={value => { const next=value === "简体中文" ? "zh-CN" : "en-US"; setLocale(next); setValue("language", next); }}/></PersonalRow>
      <PersonalRow title="Default home view" description="Select which view to display when launching Flow"><PersonalSelect label="Default home view" value={String(values.homeView)} options={["Flow Agent (default)", "Inbox", "My issues"]} onChange={v=>setValue("homeView",v)}/></PersonalRow>
      <PersonalRow title="Display names" description="Select how names are displayed in the Flow interface"><PersonalSelect label="Display names" value={String(values.displayNames)} options={["Full name","First name","Username"]} onChange={v=>setValue("displayNames",v)}/></PersonalRow>
      <PersonalRow title="First day of the week" description="Used for date pickers"><PersonalSelect label="First day of the week" value={String(values.firstDay)} options={["Monday","Saturday","Sunday"]} onChange={v=>setValue("firstDay",v)}/></PersonalRow>
      <PersonalRow title="Convert text emoticons into emojis" description="Strings like :) will be converted to 🙂"><SettingsToggle label="Convert text emoticons into emojis" checked={Boolean(values.emoticons)} onChange={v=>setValue("emoticons",v)}/></PersonalRow>
    ...
    <PersonalSection title={text.workflows}>
      <PersonalRow title="Auto-assign to self" description="When creating new issues, always assign them to yourself by default"><SettingsToggle label="Auto-assign to self" checked={Boolean(values.autoAssign)} onChange={v=>setValue("autoAssign",v)}/></PersonalRow>
      <PersonalRow title="On move to started status, assign to yourself" description="When you move an unassigned issue to started, it will be automatically assigned to you"><SettingsToggle label="On move to started status, assign to yourself" checked={Boolean(values.assignStarted)} onChange={v=>setValue("assignStarted",v)}/></PersonalRow>
    </PersonalSection>
  </>;
}
...
function Profile({ data, onReload, onBack, text }: PersonalProps) {
  ...
  return <><PersonalPageTitle>{text.profile}</PersonalPageTitle><PersonalSection>
    <PersonalRow title="Profile picture"><span className="personal-avatar">{initials(displayName)}</span></PersonalRow>
    <PersonalRow title="Email"><span className="personal-static">{data.viewer.email}</span></PersonalRow>
    <PersonalRow title="Full name"><input className="personal-input" aria-label="Full name" value={displayName} onChange={e=>setDisplayName(e.target.value)}/></PersonalRow>
    <PersonalRow title="Title" description="Your job title or role"><input className="personal-input" aria-label="Title" placeholder="Software engineer" value={jobTitle} onChange={e=>setJobTitle(e.target.value)}/></PersonalRow>
    <PersonalRow title="Username" description="One word, like a nickname or first name"><input className="personal-input" aria-label="Username" placeholder="username" value={username} onChange={e=>setUsername(e.target.value.replace(/\s+/g,""))}/></PersonalRow>
  </PersonalSection>{dirty && <div className="personal-save-bar"><Action onClick={()=>{setDisplayName(data.viewer.displayName);setUsername(current?.username||data.viewer.name);setJobTitle(current?.jobTitle||"")}}>Cancel</Action><Action primary disabled={busy||!displayName.trim()||!username.trim()} onClick={()=>void save()}>{busy?"Saving…":"Update"}</Action></div>}
  <PersonalSection title="Workspace access"><PersonalRow title="Remove yourself from workspace" danger><Action danger onClick={()=>setLeaveOpen(true)}>Leave workspace</Action></PersonalRow></PersonalSection>
...
function Notifications({ data, values, setValue, onReload, text }: PersonalProps) {
  ...
  return <><PersonalPageTitle>{text.notifications}</PersonalPageTitle>
    ...
    <PersonalSection title="Updates from Flow" description="Subscribe to product announcements and important changes from the Flow team">
      <PersonalRow title="Show updates in sidebar" description="Highlight new features and improvements in the app sidebar"><SettingsToggle label="Show updates in sidebar" checked={Boolean(values.changelogUpdates)} onChange={v=>setValue("changelogUpdates",v)}/></PersonalRow>
      <PersonalRow title="Changelog newsletter" description="Receive an email twice a month highlighting new features and improvements"><SettingsToggle label="Changelog newsletter" checked={Boolean(values.changelogNewsletter)} onChange={v=>setValue("changelogNewsletter",v)}/></PersonalRow>
    </PersonalSection>
    <PersonalSection title="Marketing"><PersonalRow title="Marketing and onboarding" description="Occasional updates to help you get the most out of Flow"><SettingsToggle label="Marketing and onboarding" checked={Boolean(values.marketingUpdates)} onChange={v=>setValue("marketingUpdates",v)}/></PersonalRow></PersonalSection>
    <PersonalSection title="Other updates">
      <PersonalRow title="Invite accepted" description="Email when invitees accept an invite"><SettingsToggle label="Invite accepted" checked={Boolean(values.inviteAcceptedUpdates)} onChange={v=>setValue("inviteAcceptedUpdates",v)}/></PersonalRow>
...
function CodeReviews({ values, setValue, text }: PersonalProps) {
  return <><PersonalPageTitle description="Review GitHub pull requests and agent code diffs in Flow">{text.codeReviews}</PersonalPageTitle>
    <PersonalSection>
      <PersonalRow title="Enable code reviews" description="Review GitHub pull requests, accessible from the sidebar"><SettingsToggle label="Enable code reviews" checked={Boolean(values.codeReviewsEnabled)} onChange={v=>setValue("codeReviewsEnabled",v)}/></PersonalRow>
      <PersonalRow title="Auto-convert draft pull requests" description="Automatically mark your drafts as ready upon approval or requesting a review"><SettingsToggle label="Auto-convert draft pull requests" checked={Boolean(values.autoConvertDrafts)} onChange={v=>setValue("autoConvertDrafts",v)}/></PersonalRow>
      <PersonalRow title="Merge strategy" description="Choose the default merge strategy for pull requests"><PersonalSelect label="Merge strategy" value={String(values.mergeStrategy)} options={["Squash and merge","Merge commit","Rebase and merge"]} onChange={v=>setValue("mergeStrategy",v)}/></PersonalRow>
    </PersonalSection>
    <PersonalSection>
      <PersonalRow title="Code theme" description="Select the syntax highlighting theme used in code diffs and viewers"><PersonalSelect label="Code theme" value={String(values.codeTheme)} options={["Flow Light","Flow Dark","GitHub Light","GitHub Dark"]} onChange={v=>setValue("codeTheme",v)}/></PersonalRow>
      <PersonalRow title="Font"><PersonalSelect label="Code font" value={String(values.codeFont)} options={["12px, Regular, Default","13px, Regular, Default","14px, Regular, Default"]} onChange={v=>setValue("codeFont",v)}/></PersonalRow>
      <pre className="personal-code-preview"><code><span>const</span> config = {"{"}\n  apiUrl: <i>"https://api.example.com"</i>,\n  timeout: <b>5000</b>,\n  debug: <em>true</em>\n{"}"};</code></pre>
    </PersonalSection>
    <PersonalSection title="Notifications" description="Choose which review activity appears in your Flow inbox and push notifications">
      <PersonalRow title="Comments & reviews" description="Comments, mentions, and submitted reviews"><PersonalSelect label="Comments & reviews" value={String(values.reviewCommentsFilter)} options={["All","Exclude Bots","Mentions only"]} onChange={v=>setValue("reviewCommentsFilter",v)}/></PersonalRow>
      <PersonalRow title="Review requests" description="Requests for your personal review"><SettingsToggle label="Review requests" checked={Boolean(values.reviewRequests)} onChange={v=>setValue("reviewRequests",v)}/></PersonalRow>
      <PersonalRow title="GitHub team review requests" description="Requests for review from your GitHub teams with 10 or fewer members"><SettingsToggle label="GitHub team review requests" checked={Boolean(values.githubTeamReviewRequests)} onChange={v=>setValue("githubTeamReviewRequests",v)}/></PersonalRow>
      <PersonalRow title="Checks & merge queue" description="Check failures and merge queue updates"><SettingsToggle label="Checks & merge queue" checked={Boolean(values.checksMergeQueue)} onChange={v=>setValue("checksMergeQueue",v)}/></PersonalRow>
    </PersonalSection>
    <PersonalSection title="Signed commits">
      <PersonalRow title="Require signed commits" description="Users must upload a signing key before starting a coding session"><SettingsToggle label="Require signed commits" checked={Boolean(values.requireSignedCommits)} onChange={v=>setValue("requireSignedCommits",v)} disabled/></PersonalRow>
      <PersonalRow title="No signing key added"><Action disabled>Add key</Action></PersonalRow>
    </PersonalSection>
    <PersonalSection title="External tools">
      <PersonalRow title="Configure coding tools" description="Configure the external coding tools you can open issues in"><ExternalLink size={16}/></PersonalRow>
      <PersonalRow title="Git attachment format" description="The format of GitHub/GitLab attachments on issues"><PersonalSelect label="Git attachment format" value={String(values.gitAttachmentFormat)} options={["Title","URL","Title and URL"]} onChange={v=>setValue("gitAttachmentFormat",v)}/></PersonalRow>
      <PersonalRow title="On git branch copy, move issue to started status" description="After copying the git branch name, issue status is moved to the team’s first started workflow status."><SettingsToggle label="On git branch copy, move issue to started status" checked={Boolean(values.gitBranchMoveStarted)} onChange={v=>setValue("gitBranchMoveStarted",v)}/></PersonalRow>
      <PersonalRow title="On open in coding tool, move issue to started status" description="After opening an issue in a coding tool or copying as prompt, issue status is moved to the team’s first started workflow status."><SettingsToggle label="On open in coding tool, move issue to started status" checked={Boolean(values.codingToolMoveStarted)} onChange={v=>setValue("codingToolMoveStarted",v)}/></PersonalRow>
    </PersonalSection>
  </>;
}
...
function Security({ data, onNavigate, onReload, text }: PersonalProps) {
  ...
  return <><PersonalPageTitle>{text.security}</PersonalPageTitle>
    ...
    <PersonalSection title="Authorized applications">{data.oauthAuthorizations.filter(item=>item.userId===data.viewer.id&&!item.revokedAt).map(item=><PersonalRow key={item.id} icon={<ShieldCheck/>} title={item.clientName} description={`MCP · ${item.scopes.join(', ')}`}><Action danger onClick={()=>void revokeOAuthAuthorization(item.id).then(onReload)}>Revoke</Action></PersonalRow>)}{!data.oauthAuthorizations.some(item=>item.userId===data.viewer.id&&!item.revokedAt)&&<div className="personal-empty"><ShieldCheck/><h3>No authorized applications</h3></div>}</PersonalSection>
