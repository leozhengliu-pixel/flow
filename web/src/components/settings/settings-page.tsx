...
export function SettingsPage(props: SettingsPageProps) {
  ...
  const [sidebarCustomizationOpen, setSidebarCustomizationOpen] = useState(false);
    ...
    <main className="settings-main">
      ...
      <div className="settings-content">
        <Suspense fallback={<SettingsPageFallback/>}>
          <SettingsBody {...props} settings={settings} setSettings={setSettings} setValue={setValue} onCustomizeSidebar={()=>setSidebarCustomizationOpen(true)}/>
        </Suspense>
      </div>
    </main>
  </div><SidebarCustomization
...
function SettingsBody(props: SettingsPageProps & { settings: StoredSettings; setSettings: React.Dispatch<React.SetStateAction<StoredSettings>>; setValue: (key: string, value: string | boolean) => void; onCustomizeSidebar: () => void }) {
  const { t } = useI18n();
  const { page } = props;
  ...
  if (personal) return <PersonalSettings page={page} data={props.data} values={props.settings.values} setValue={props.setValue} onNavigate={props.onNavigate} onReload={props.onReload} onBack={props.onBack} onCustomizeSidebar={props.onCustomizeSidebar}/>;
  ...
  if (page === "members") return <MembersPageV2 data={props.data} onReload={props.onReload}/>;
  ...
  if (page === "security") return <><SecurityPage data={props.data} onReload={props.onReload}/><SecuritySupplement data={props.data} onNavigate={props.onNavigate} onReload={props.onReload}/></>;
...
export function MembersPage({ data, onReload }: { data: BootstrapData; onReload: () => Promise<void> }) {
  const [query, setQuery] = useState("");
...
function SecuritySupplement({data,onNavigate,onReload}:{data:BootstrapData;onNavigate:(page:SettingsPageId)=>void;onReload:()=>Promise<void>}){
  const settings=data.workspaceSettings
  const save=async(patch:Partial<WorkspaceSettings>)=>{try{await updateWorkspacePreferences({...settings,...patch});await onReload();toast.success('Security policy saved')}catch(error){toast.error(error instanceof Error?error.message:'Could not save policy')}}
  const enterprise=settings.plan==='enterprise'
  return <>
    <Section title="File uploads"><Row title="Restrict file uploads" description="Restrict uploaded file types. Images and videos remain allowed."><ActionButton disabled={!enterprise}>{enterprise?'Configure':'Available on Enterprise'}</ActionButton></Row></Section>
    <Section title="Application approvals"><Row title="Review third-party applications" description="Control which applications can be installed to your workspace"><Toggle label="Review third-party applications" checked={Boolean(settings.reviewThirdPartyApplications)} onChange={value=>void save({reviewThirdPartyApplications:value})}/></Row><Row title="Reduce personal information from support integrations" description="Personal information from support integrations will not be stored"><ActionButton disabled={!enterprise}>{enterprise?'Configure':'Available on Enterprise'}</ActionButton></Row></Section>
    <Section title="MCP connections"><Row title="Active MCP connections" description={`${data.oauthAuthorizations.filter(item=>!item.revokedAt).length} active connections across workspace members`}><ActionButton onClick={()=>onNavigate('applications')}>Review</ActionButton></Row><Row title="Allowed MCP connectors" description="Choose which MCP connectors Flow Agent can use"><Select label="Allowed MCP connectors" value={(settings.allowedMcpConnectors??'all')==='all'?'All connectors':'Approved connectors'} options={['All connectors','Approved connectors']} onChange={value=>void save({allowedMcpConnectors:value==='All connectors'?'all':'approved'})}/></Row></Section>
    <Section title="Compliance"><Row title="HIPAA compliance" description="Enable privacy and security measures for protected health information"><ActionButton disabled={!enterprise}>{enterprise?'Configure':'Available on Enterprise'}</ActionButton></Row></Section>
  </>
}

function MembersPageV2({data,onReload}:{data:BootstrapData;onReload:()=>Promise<void>}) {
  const {t}=useI18n()
  const [query,setQuery]=useState('')
  const [status,setStatus]=useState('All')
  const [sort,setSort]=useState<'name'|'email'|'status'|'joined'|'lastSeen'>('name')
  const [descending,setDescending]=useState(false)
  const [inviteOpen,setInviteOpen]=useState(false)
  const [emails,setEmails]=useState('')
  const [inviteRole,setInviteRole]=useState<'admin'|'member'|'guest'>('member')
  const [inviteTeams,setInviteTeams]=useState<string[]>(data.teams.map(team=>team.id))
  const [roleTarget,setRoleTarget]=useState<WorkspaceMember>()
  const [roleDraft,setRoleDraft]=useState<'admin'|'member'|'guest'>('member')
  const [identityTarget,setIdentityTarget]=useState<{member:WorkspaceMember;field:'displayName'|'username'|'email';value:string}>()
  const [teamsTarget,setTeamsTarget]=useState<WorkspaceMember>()
  const [teamDraft,setTeamDraft]=useState<string[]>([])
  const [confirmTarget,setConfirmTarget]=useState<{member:WorkspaceMember;action:'suspend'|'remove'|'resume'}>()
  const [busy,setBusy]=useState(false)
  const normalized=query.trim().toLowerCase()
  const visibleMembers=data.members.filter(member=>{
    const matchesQuery=!normalized||`${member.user.displayName} ${member.user.name} ${member.user.email}`.toLowerCase().includes(normalized)
    const matchesStatus=status==='All'||status==='Admins'&&member.role==='admin'&&member.status==='active'||status==='Members'&&member.role==='member'&&member.status==='active'||status==='Guests'&&member.role==='guest'&&member.status==='active'||status==='Suspended'&&member.status==='suspended'
    return matchesQuery&&matchesStatus
  }).sort((left,right)=>{
    const value=(member:WorkspaceMember)=>sort==='name'?member.user.displayName:sort==='email'?member.user.email:sort==='status'?`${member.status}:${member.role}`:sort==='joined'?member.joinedAt:member.lastSeenAt??''
    return value(left).localeCompare(value(right))*(descending?-1:1)
  })
  const pending=data.invitations.filter(invitation=>invitation.status==='pending'&&(status==='All'||status==='Pending invites')&&(!normalized||invitation.email.toLowerCase().includes(normalized)))
  const applications=status==='All'||status==='Applications'?data.oauthApplications.filter(application=>!normalized||application.name.toLowerCase().includes(normalized)):[]
  const active=visibleMembers.filter(member=>member.status==='active'),suspended=visibleMembers.filter(member=>member.status==='suspended')
  const change=async(action:()=>Promise<unknown>,success:string)=>{setBusy(true);try{await action();await onReload();toast.success(t(success))}catch(error){toast.error(error instanceof Error?error.message:t('Could not update member'))}finally{setBusy(false)}}
  const order=(key:typeof sort)=>{if(sort===key)setDescending(value=>!value);else{setSort(key);setDescending(false)}}
  const sendInvites=()=>change(async()=>{const parsed=emails.split(/[\s,;]+/).map(value=>value.trim()).filter(Boolean);if(!parsed.length)throw new Error(t('Enter at least one email address'));if(inviteRole==='guest'&&!inviteTeams.length)throw new Error(t('Guests must be assigned to a team'));await inviteMembers(data.workspace.urlKey,{emails:parsed,role:inviteRole,teamIds:inviteTeams});setInviteOpen(false);setEmails('')},'Invitation sent')
  const exportCsv=()=>{const quote=(value:string)=>`"${value.replaceAll('"','""')}"`;download('members.csv',[['Name','Username','Email','Status','Role','Teams','Joined','Last seen'].map(quote).join(','),...data.members.map(member=>[member.user.displayName,member.user.name,member.user.email,member.status,member.role,String(data.teamMembers.filter(team=>team.userId===member.user.id).length),member.joinedAt,member.lastSeenAt??''].map(quote).join(','))].join('\n'),'text/csv')}
  const header=<div className="settings-members-columns"><button onClick={()=>order('name')}>{t('Name')}</button><button onClick={()=>order('email')}>{t('Email')}</button><button onClick={()=>order('status')}>{t('Status')}</button><span>{t('Teams')}</span><button onClick={()=>order('joined')}>{t('Joined')}</button><button onClick={()=>order('lastSeen')}>{t('Last seen')}</button><span/></div>
  const memberRow=(member:WorkspaceMember)=>{const memberships=data.teamMembers.filter(item=>item.userId===member.user.id);return <div className="settings-member-directory-row" key={member.user.id}><span><b className="settings-member-avatar">{initials(member.user.displayName)}</b><i><strong data-i18n-ignore>{member.user.displayName}</strong><small data-i18n-ignore>{member.user.name}</small></i></span><span data-i18n-ignore>{member.user.email}</span><span><em className={`settings-member-role is-${member.role}`}>{t(title(member.role))}{member.status==='suspended'?` (${t('Suspended')})`:''}</em></span><span><button className="settings-member-teams" onClick={()=>{setTeamsTarget(member);setTeamDraft(memberships.map(item=>item.teamId))}}>{memberships.length?`${memberships.length} ${t(memberships.length===1?'team':'teams')}`:t('No teams')}</button></span><time>{formatDate(member.joinedAt)}</time><span>{member.user.id===data.viewer.id?<><i className="settings-member-online"/>{t('Online')}</>:member.lastSeenAt?formatDate(member.lastSeenAt):t('Never')}</span><span><DropdownMenu><DropdownMenuTrigger asChild><button className="settings-member-more" aria-label={`${t('Open menu')} ${member.user.displayName}`}><MoreHorizontal size={15}/></button></DropdownMenuTrigger><DropdownMenuContent align="end" className="settings-member-action-menu" data-i18n-ignore><DropdownMenuItem onSelect={()=>{setRoleTarget(member);setRoleDraft(member.role as typeof roleDraft)}}>{t('Change role…')}</DropdownMenuItem><DropdownMenuSeparator/><DropdownMenuItem onSelect={()=>setIdentityTarget({member,field:'displayName',value:member.user.displayName})}>{t('Update name…')}</DropdownMenuItem><DropdownMenuItem onSelect={()=>setIdentityTarget({member,field:'username',value:member.user.name})}>{t('Update username…')}</DropdownMenuItem><DropdownMenuItem onSelect={()=>setIdentityTarget({member,field:'email',value:member.user.email})}>{t('Update email…')}</DropdownMenuItem><DropdownMenuSeparator/>{member.status==='suspended'?<DropdownMenuItem onSelect={()=>setConfirmTarget({member,action:'resume'})}>{t('Restore user…')}</DropdownMenuItem>:<DropdownMenuItem disabled={member.user.id===data.viewer.id} onSelect={()=>setConfirmTarget({member,action:'suspend'})}>{t('Suspend user…')}</DropdownMenuItem>}<DropdownMenuSeparator/><DropdownMenuItem onSelect={()=>{setTeamsTarget(member);setTeamDraft(memberships.map(item=>item.teamId))}}>{t('Manage teams…')}</DropdownMenuItem><DropdownMenuItem className="danger-item" disabled={member.user.id===data.viewer.id} onSelect={()=>setConfirmTarget({member,action:'remove'})}>{t('Remove from workspace…')}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></span></div>}
  return <div className="settings-members-page" data-i18n-ignore><PageTitle>{t('Members')}</PageTitle><div className="settings-members-toolbar"><label><Search size={14}/><input aria-label={t('Search by name or email')} placeholder={t('Search by name or email')} value={query} onChange={event=>setQuery(event.target.value)}/></label><Select label={t('Member status')} value={t(status)} options={['All','Admins','Members','Guests','Applications','Pending invites','Suspended','Left workspace'].map(t)} onChange={value=>{const values=['All','Admins','Members','Guests','Applications','Pending invites','Suspended','Left workspace'];setStatus(values.find(item=>t(item)===value)??'All')}}/><span/><ActionButton onClick={exportCsv}>{t('Export CSV')}</ActionButton><ActionButton primary onClick={()=>setInviteOpen(true)}><Plus size={14}/>{t('Invite')}</ActionButton></div><div className="settings-members-directory">{header}{active.length>0&&<><div className="settings-members-group"><strong>{t('Active')}</strong><span>{active.length}</span></div>{active.map(memberRow)}</>}{suspended.length>0&&<><div className="settings-members-group"><strong>{t('Suspended')}</strong><span>{suspended.length}</span></div>{suspended.map(memberRow)}</>}{pending.length>0&&<><div className="settings-members-group"><strong>{t('Invited')}</strong><span>{pending.length}</span></div>{pending.map(invitation=><div className="settings-member-directory-row is-invited" key={invitation.id}><span><b className="settings-member-avatar is-invited">{initials(invitation.email)}</b><i><strong data-i18n-ignore>{invitation.email}</strong><small data-i18n-ignore>{invitation.email}</small></i></span><span data-i18n-ignore>{invitation.email}</span><span><em className={`settings-member-role is-${invitation.role}`}>{t(title(invitation.role))} ({t('Invited')})</em></span><span>{invitation.teamIds.length} {t(invitation.teamIds.length===1?'team':'teams')}</span><time>{formatDate(invitation.createdAt)}</time><span>—</span><span><DropdownMenu><DropdownMenuTrigger asChild><button className="settings-member-more" aria-label={t('Open menu')}><MoreHorizontal size={15}/></button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={()=>void change(()=>resendInvitation(data.workspace.urlKey,invitation.id),'Invitation resent')}>{t('Resend invite')}</DropdownMenuItem><DropdownMenuItem className="danger-item" onSelect={()=>void change(()=>revokeInvitation(data.workspace.urlKey,invitation.id),'Invitation revoked')}>{t('Revoke invite')}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></span></div>)}</>}{applications.length>0&&<><div className="settings-members-group"><strong>{t('Application')}</strong><span>{applications.length}</span></div>{applications.map(application=><div className="settings-member-directory-row is-application" key={application.id}><span><b className="settings-member-avatar">AP</b><i><strong data-i18n-ignore>{application.name}</strong><small>application</small></i></span><span>—</span><span>{t('Application')}</span><span>—</span><time>—</time><span>—</span><span/></div>)}</>}{!active.length&&!suspended.length&&!pending.length&&!applications.length&&<div className="settings-empty compact"><UserRound size={24}/><h3>{t('No members found')}</h3><p>{t('Try another name, email, or member status.')}</p></div>}</div>
    <Dialog open={inviteOpen} onOpenChange={setInviteOpen}><DialogContent className="settings-invite-dialog"><DialogTitle><span className="settings-invite-mark">{data.workspace.name.slice(0,2).toUpperCase()}</span>{t('Invite to your workspace')}</DialogTitle><label>{t('Email')}<textarea autoFocus aria-label={t('Email')} placeholder="email@foxmail.com, email2@foxmail.com…" value={emails} onChange={event=>setEmails(event.target.value)}/></label><label>{t('Role')}<Select label={t('Invitation role')} value={t(title(inviteRole))} options={['Member','Admin','Guest'].map(t)} onChange={value=>setInviteRole((['member','admin','guest'] as const)[['Member','Admin','Guest'].map(t).indexOf(value)]??'member')}/></label>{inviteRole==='guest'&&<label>{t('Teams')}<button className="settings-select" onClick={()=>setInviteTeams(current=>current.length?[]:data.teams.map(team=>team.id))}>{inviteTeams.length?`${inviteTeams.length} ${t('selected')}`:t('Select teams')}</button></label>}<footer><ActionButton primary disabled={busy||!emails.trim()} onClick={()=>void sendInvites()}>{busy?t('Sending…'):t('Send invites')}</ActionButton></footer></DialogContent></Dialog>
    <Dialog open={Boolean(roleTarget)} onOpenChange={open=>!open&&setRoleTarget(undefined)}><DialogContent className="settings-member-dialog"><DialogTitle>{t('Change role')}</DialogTitle>{(['admin','member','guest'] as const).map(value=><label className="settings-member-role-option" key={value}><input type="radio" checked={roleDraft===value} onChange={()=>setRoleDraft(value)}/><span><strong>{t(value==='admin'?'Workspace admin':title(value))}</strong><small>{t(value==='admin'?'Full control of the workspace including security, billing, and all settings':value==='member'?'Standard workspace access with the ability to act within all public teams':'Access limited to specific teams, with no workspace views or features')}</small></span></label>)}<footer><ActionButton onClick={()=>setRoleTarget(undefined)}>{t('Cancel')}</ActionButton><ActionButton primary disabled={busy} onClick={()=>roleTarget&&void change(()=>updateMemberRole(data.workspace.urlKey,roleTarget.user.id,roleDraft),'Role updated').then(()=>setRoleTarget(undefined))}>{t('Save')}</ActionButton></footer></DialogContent></Dialog>
    <Dialog open={Boolean(identityTarget)} onOpenChange={open=>!open&&setIdentityTarget(undefined)}><DialogContent className="settings-member-dialog"><DialogTitle>{t(identityTarget?.field==='displayName'?'Update name':identityTarget?.field==='username'?'Update username':'Update email')}</DialogTitle><input autoFocus className="settings-input" value={identityTarget?.value??''} onChange={event=>setIdentityTarget(current=>current?{...current,value:event.target.value}:current)}/><footer><ActionButton onClick={()=>setIdentityTarget(undefined)}>{t('Cancel')}</ActionButton><ActionButton primary disabled={busy||!identityTarget?.value.trim()} onClick={()=>identityTarget&&void change(()=>updateMemberIdentity(data.workspace.urlKey,identityTarget.member.user.id,{[identityTarget.field]:identityTarget.value.trim()}),'Member updated').then(()=>setIdentityTarget(undefined))}>{t('Save')}</ActionButton></footer></DialogContent></Dialog>
    <Dialog open={Boolean(teamsTarget)} onOpenChange={open=>!open&&setTeamsTarget(undefined)}><DialogContent className="settings-member-dialog"><DialogTitle>{t('Manage teams')}</DialogTitle>{data.teams.map(team=><label className="settings-member-team-option" key={team.id}><input type="checkbox" checked={teamDraft.includes(team.id)} onChange={event=>setTeamDraft(current=>event.target.checked?[...current,team.id]:current.filter(id=>id!==team.id))}/><span data-i18n-ignore>{team.name}</span></label>)}<footer><ActionButton onClick={()=>setTeamsTarget(undefined)}>{t('Cancel')}</ActionButton><ActionButton primary disabled={busy} onClick={()=>teamsTarget&&void change(()=>Promise.all(data.teams.map(team=>{const current=data.teamMembers.some(item=>item.teamId===team.id&&item.userId===teamsTarget.user.id),next=teamDraft.includes(team.id);return current===next?Promise.resolve():setTeamMembership(data.workspace.urlKey,team.id,teamsTarget.user.id,next,'member')})),'Team access updated').then(()=>setTeamsTarget(undefined))}>{t('Save')}</ActionButton></footer></DialogContent></Dialog>
    <ConfirmDialog open={Boolean(confirmTarget)} title={confirmTarget?`${t(confirmTarget.action==='resume'?'Restore':'Suspend')} ${confirmTarget.member.user.displayName}?`:''} description={confirmTarget?.action==='suspend'?t("They won’t be able to access this workspace."):confirmTarget?.action==='remove'?t('This member will lose access to the workspace and all teams.'):t('This member will regain access to the workspace.')} confirm={t(confirmTarget?.action==='remove'?'Remove':confirmTarget?.action==='resume'?'Restore':'Confirm')} onCancel={()=>setConfirmTarget(undefined)} onConfirm={()=>{if(!confirmTarget)return;const action=confirmTarget.action==='suspend'?suspendMember(data.workspace.urlKey,confirmTarget.member.user.id):confirmTarget.action==='resume'?resumeMember(data.workspace.urlKey,confirmTarget.member.user.id):removeMember(data.workspace.urlKey,confirmTarget.member.user.id);void change(()=>action,confirmTarget.action==='resume'?'Member restored':confirmTarget.action==='remove'?'Member removed':'Member suspended').then(()=>setConfirmTarget(undefined))}}/>
  </div>
}

function SecurityPage({data,onReload}:{data:BootstrapData;onReload:()=>Promise<void>}){const[settings,setSettings]=useState(data.workspaceSettings);useEffect(()=>setSettings(data.workspaceSettings),[data.workspaceSettings]);const save=async(next:WorkspaceSettings)=>{setSettings(next);try{await updateWorkspacePreferences(next);await onReload();toast.success('Security policy saved')}catch(error){setSettings(data.workspaceSettings);toast.error(error instanceof Error?error.message:'Could not save policy')}};const toggle=(title:string,key:keyof WorkspaceSettings,description:string)=><Row title={title} description={description}><Toggle label={title} checked={Boolean(settings[key])} onChange={value=>void save({...settings,[key]:value})}/></Row>;return <><PageTitle description="Manage authentication, permissions, and access policies for your workspace.">Security</PageTitle><Section title="Workspace access">{toggle('Invite links','inviteLinksEnabled','Allow members to invite people with a workspace link.')}<Row title="Allow guest accounts" description="Guest invitations are rejected when this is disabled"><Toggle label="Allow guest accounts" checked={settings.guestsAllowed} onChange={value=>void save({...settings,guestsAllowed:value})}/></Row></Section><Section title="Authentication methods">{toggle('Google authentication','googleAuthEnabled','When enabled, this is available to all workspace members and guests')}{toggle('Email & passkey authentication','emailAuthEnabled','When enabled, this is available to all workspace members and guests')}<Row title="Require two-factor authentication" description="Require a second factor for all members."><Toggle label="Require two-factor authentication" checked={settings.requireTwoFactor} onChange={value=>void save({...settings,requireTwoFactor:value})}/></Row>{toggle('Disable authentication bypass for admins','disableAdminBypass','When enabled, admins are restricted to the enabled authentication methods')}<Row title="Session duration"><Select label="Session duration" value={`${settings.sessionDurationDays} days`} options={["7 days","30 days","90 days"]} onChange={value=>void save({...settings,sessionDurationDays:Number(value.split(' ')[0])})}/></Row></Section><EnterpriseIdentityProviders data={data} onReload={onReload}/><Section title="Workspace login and restrictions"><FieldRow title="Approved email domains" description="Anyone with an email address at these domains is allowed to sign up for this workspace." value={(settings.allowedDomains??[]).join(', ')} onCommit={value=>void save({...settings,allowedDomains:value.split(',').map(item=>item.trim()).filter(Boolean)})}/></Section><Section title="Workspace management"><PermissionRow title="New user invitations" value={settings.invitePermission} onChange={value=>void save({...settings,invitePermission:value})}/><PermissionRow title="Team creation" value={settings.teamCreatePermission} onChange={value=>void save({...settings,teamCreatePermission:value})}/><PermissionRow title="Manage workspace labels" value={settings.labelPermission} onChange={value=>void save({...settings,labelPermission:value})}/><PermissionRow title="Manage workspace templates" value={settings.templatePermission} onChange={value=>void save({...settings,templatePermission:value})}/><PermissionRow title="Workspace initiatives" value={settings.initiativePermission??'members'} onChange={value=>void save({...settings,initiativePermission:value})}/><PermissionRow title="Manage loops" value={settings.loopPermission??'members'} onChange={value=>void save({...settings,loopPermission:value})}/><PermissionRow title="Modify agent guidance" value={settings.agentGuidancePermission??'admins'} onChange={value=>void save({...settings,agentGuidancePermission:value})}/><PermissionRow title="API key creation" value={settings.apiKeyPermission} onChange={value=>void save({...settings,apiKeyPermission:value})}/></Section><Section title="Integrations & applications">{toggle('Prevent guests from interacting with agents in the workspace','preventGuestAgents','Restrict agent invocation to full workspace members only')}{toggle('Improve AI features by sharing usage data','aiUsageSharing','Feedback on AI results is used to enhance functionality')}{toggle('Enable Flow Agent web search','agentWebSearch','Allow Flow Agent to search the public web for current information and cite sources')}{toggle('Allow external sources to trigger loops','externalLoopTriggers','Select which external sources can trigger loops')}{toggle('Enable Flow Agent MCP connectors','mcpConnectorsEnabled','Allow Flow Agent to use MCP connectors')}</Section></>}
...
function UsagePage({ data }: { data: BootstrapData }) {
  const { t } = useI18n(); const [usage,setUsage]=useState<Awaited<ReturnType<typeof fetchWorkspaceUsage>>|null>(null); const [alerts,setAlerts]=useState<UsageAlert[]>([]); const [subscription,setSubscription]=useState<PaidSubscription|null>(null); const [weekOffset,setWeekOffset]=useState(0)
  const load=async()=>{const[value,alertPage,paid]=await Promise.all([fetchWorkspaceUsage(),listUsageAlerts(),getPaidSubscription()]);setUsage(value);setAlerts(alertPage.nodes);setSubscription(paid)}
  useEffect(()=>{void load()},[data])
  if(!usage)return <div className="settings-empty compact"><Gauge size={24}/><p>{t('Loading usage…')}</p></div>
  const save=async(patch:Partial<WorkspaceSettings>)=>{await updateWorkspacePreferences({...data.workspaceSettings,...patch});await load()}
  const now=new Date(); const end=new Date(now); end.setDate(now.getDate()+weekOffset*7); const start=new Date(end); start.setDate(end.getDate()-6); const periodEvents=usage.events.filter(event=>{const date=new Date(event.createdAt);return date>=start&&date<=new Date(end.getFullYear(),end.getMonth(),end.getDate(),23,59,59)}); const coding=periodEvents.filter(event=>event.feature==='coding-sessions').reduce((sum,event)=>sum+event.amountCents,0); const loops=periodEvents.filter(event=>event.feature==='loops').reduce((sum,event)=>sum+event.amountCents,0); const maxSpend=Math.max(1,coding,loops)
  return <div className="settings-usage-page"><PageTitle description={t('Track usage, manage credits, and set spend limits across your workspace')}>{t('Usage & limits')}</PageTitle>
    {alerts.filter(alert=>alert.status==='active').map(alert=><div className="settings-usage-alert" key={alert.id}><Gauge/><span><strong>{t('Usage alert')}</strong><small>{t(alert.type)}: {alert.current} / {alert.threshold}</small></span></div>)}
    <Section title={t('AI credits')}><div className="settings-credit-summary"><span>{t('Workspace credits')}</span><strong>{currency(usage.aiCredits.balanceCents)} <small>{t('available')}</small></strong></div><Row title={t('Automatic reload')} description={t('Automatically top up credits when your balance runs low')}><Toggle label={t('Automatic credit reload')} checked={usage.aiCredits.autoReloadEnabled} onChange={value=>void save({aiCreditAutoReload:value})}/></Row>{usage.aiCredits.autoReloadEnabled&&<div className="settings-reload-controls"><Row title={t('Reload threshold')}><NumberInput value={usage.aiCredits.autoReloadThresholdCents} onCommit={value=>save({aiCreditReloadThresholdCents:value})}/></Row><Row title={t('Reload amount')}><NumberInput value={usage.aiCredits.autoReloadAmountCents} onCommit={value=>save({aiCreditReloadAmountCents:value})}/></Row></div>}</Section>
    <Section><Row title={t('Spend limits')} description={t('Set workspace, user, and loop limits')}><NumberInput value={usage.aiCredits.workspaceSpendLimitCents} onCommit={value=>save({aiWorkspaceSpendLimitCents:value})}/></Row></Section>
    <section className="settings-usage-analytics"><header><h3>{t('Analytics')}</h3><div><button type="button">{t('Week')}</button><button aria-label={t('Previous period')} type="button" onClick={()=>setWeekOffset(value=>value-1)}><ChevronLeft/></button><span>{start.toLocaleDateString(undefined,{month:'short',day:'numeric'})} – {end.toLocaleDateString(undefined,{month:'short',day:'numeric'})}</span><button aria-label={t('Next period')} disabled={weekOffset===0} type="button" onClick={()=>setWeekOffset(value=>Math.min(0,value+1))}><ChevronRight/></button></div></header><strong className="settings-usage-total">{currency(coding+loops)}<small>{t('total spend in this period')}</small></strong><div className="settings-usage-chart"><i style={{height:`${Math.max(2,coding/maxSpend*100)}%`}}/><i style={{height:`${Math.max(2,loops/maxSpend*100)}%`}}/></div><div className="settings-usage-series"><span><i/>{t('Coding sessions')}<strong>{currency(coding)}</strong></span><span><i/>{t('Loops')}<strong>{currency(loops)}</strong></span></div>{!periodEvents.length&&<div className="settings-usage-no-data"><Activity/><span>{t('No usage this week')}</span></div>}</section>
    <Section title={t('Plan usage')}>{subscription&&<Row title={title(subscription.plan)} description={`${subscription.seats} ${t('seats')} · ${subscription.status}`}><span className="settings-static">{subscription.cancelAtPeriodEnd?t('Cancels at period end'):t('Active')}</span></Row>}<UsageRow title={t('Members')} value={usage.members} limit={usage.limits.members}/><UsageRow title={t('Issues')} value={usage.issues} limit={usage.limits.issues}/><UsageRow title={t('File storage (MB)')} value={Math.ceil(usage.storageBytes/1048576)} limit={Math.ceil(usage.limits.storageBytes/1048576)}/></Section>
  </div>
}
