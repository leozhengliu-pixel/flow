...
import { SettingsToggle as Toggle } from './settings-primitives'
...
export function FeatureSettingsPage({ page, data, onCreateReleasePipeline, onOpenReleasePipeline, onOpenIntegration, onReload }: Props) {
  ...
  if (page === "asks") return <AsksFeatureSettings data={data} settings={settings} busy={busy} setFeature={setFeature} onReload={onReload}/>;
...
function DomainDialog({title,values,onClose,onSave}:{title:string;values:string[];onClose:()=>void;onSave:(values:string[])=>void}) {const [text,setText]=useState(values.join("\n"));return <FeatureDialog open onClose={onClose} title={title}><label>One domain or email per line<textarea autoFocus value={text} onChange={event=>setText(event.target.value)}/></label><FeatureDialogFooter><span/><FeatureButton onClick={onClose}>Cancel</FeatureButton><FeatureButton primary onClick={()=>onSave([...new Set(text.split(/[\n,]+/).map(value=>value.trim().toLowerCase()).filter(Boolean))])}>Save</FeatureButton></FeatureDialogFooter></FeatureDialog>}
