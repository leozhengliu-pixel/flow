...
function EmojisPage({data,onReload}:{data:BootstrapData;onReload:()=>Promise<void>}) {
  ...
  const choose=(file?:File)=>{if(!file)return;if(file.size>500_000){toast.error(t("Emoji images must be smaller than 500 KB"));return}const reader=new FileReader();reader.onload=()=>setUpload({name:file.name.replace(/\.[^.]+$/,"").toLowerCase().replace(/[^a-z0-9_-]+/g,"-"),imageUrl:String(reader.result)});reader.readAsDataURL(file)};
