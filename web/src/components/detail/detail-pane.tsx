...
export function DetailPane({issue,data,comments,activities,presence=[],full=false,embedded=false,issueOptionsActions,onClose,onNavigateIssue,onUpdate,onDelete,onCreateSubIssue,onReactIssue,onComment,onEditComment,onDeleteComment,onReactComment,onRelation,onDeleteRelation,onUpload,onDeleteAttachment}:{issue:Issue;data:BootstrapData;comments:Comment[];activities:ActivityEvent[];presence?:Presence[];full?:boolean;embedded?:boolean;issueOptionsActions?:IssueOptionsActions;onClose:()=>void;onExpand?:()=>void;onNavigateIssue?:(issue:Issue)=>void;onUpdate:(input:IssueUpdateInput)=>Promise<void>;onDelete:()=>Promise<void>;onCreateSubIssue:(input:SubIssueInput)=>Promise<void>;onReactIssue:(emoji:string)=>Promise<void>;onComment:(body:string,bodyData?:Record<string,unknown>,parentId?:string)=>Promise<void>;onEditComment:(id:string,body:string,bodyData?:Record<string,unknown>)=>Promise<void>;onDeleteComment:(id:string)=>Promise<void>;onReactComment:(id:string,emoji:string)=>Promise<void>;onRelation:(type:IssueRelationType,relatedIssueId:string)=>Promise<void>;onDeleteRelation:(relationId:string)=>Promise<void>;onUpload:(file:File)=>Promise<void>;onDeleteAttachment:(attachmentId:string)=>Promise<void>}){
  ...
  const availableLabels=data.labels.filter(label=>!label.scope||label.scope==='Workspace'||label.scope===issue.team.id)
...
function IssueProperties({issue,data,activities,related,onUpdate,onToggleLabel}:{issue:Issue;data:BootstrapData;activities:ActivityEvent[];related:(id:string)=>Issue|undefined;onUpdate:(i:IssueUpdateInput)=>Promise<void>;onToggleLabel:(id:string)=>Promise<void>}){const labels=data.labels.filter(label=>!label.scope||label.scope==='Workspace'||label.scope===issue.team.id);const sla=data.issueSlas.find(item=>item.issueId===issue.id&&item.status!=='removed');const rule=sla?data.slaRules.find(item=>item.id===sla.ruleId):undefined;const member=issue.assignee?data.members.find(item=>item.user.id===issue.assignee?.id):undefined;return <aside className="issue-properties"><h3>Properties</h3><StatusPicker value={issue.state} states={statesForIssue(data,issue)} hoverHistory={{activities,issueCreatedAt:issue.createdAt}} onChange={stateId=>onUpdate({stateId})}/><PriorityPicker value={issue.priority} onChange={priority=>onUpdate({priority})}/><div className="issue-assignee-property"><AssigneePicker value={issue.assignee} users={data.users} hoverContext={{member,workspaceName:issue.team.name,project:issue.project}} onChange={assigneeId=>onUpdate({assigneeId})}/>{issue.assignee&&<a className="issue-assignee-profile" href={`/${data.workspace.urlKey}/members`} aria-label="Go to user"><ChevronRight size={15}/></a>}</div>{sla&&<PropertyGroup title="SLA"><IssueSLAIndicator sla={sla} ruleName={rule?.name}/></PropertyGroup>}<PropertyGroup title="Labels"><LabelPicker inline value={issue.labels} labels={labels} onToggle={onToggleLabel}/></PropertyGroup><PropertyGroup title="Project"><ProjectPicker value={issue.project} projects={data.projects} teamName={issue.team.name} onChange={projectId=>onUpdate({projectId})}/></PropertyGroup>{issue.parentId&&<PropertyGroup title="Parent"><PropertyMenu compact label="Parent" value={`${related(issue.parentId)?.identifier} ${related(issue.parentId)?.title}`} options={[{id:'',label:'No parent'},...data.issues.filter(i=>i.id!==issue.id).map(i=>({id:i.id,label:`${i.identifier} ${i.title}`}))]} onChange={parentId=>onUpdate({parentId})}/></PropertyGroup>}{issue.dueDate&&<PropertyGroup title="Due date"><DueDatePicker value={issue.dueDate} onChange={dueDate=>onUpdate({dueDate})}/></PropertyGroup>}</aside>}
