import { ChevronDown, ChevronRight, FileText, Link2, Paperclip, Plus, SmilePlus, UserRound, X } from 'lucide-react'
...
export function DetailPane({issue,data,comments,activities,presence=[],full=false,embedded=false,issueOptionsActions,onClose,onNavigateIssue,onNavigateRoot,onUpdate,onDelete,onCreateSubIssue,onReactIssue,onComment,onEditComment,onDeleteComment,onReactComment,onRelation,onDeleteRelation,onUpload,onDeleteAttachment}:{issue:Issue;data:BootstrapData;comments:Comment[];activities:ActivityEvent[];presence?:Presence[];full?:boolean;embedded?:boolean;issueOptionsActions?:IssueOptionsActions;onClose:()=>void;onExpand?:()=>void;onNavigateIssue?:(issue:Issue)=>void;onNavigateRoot?:()=>void;onUpdate:(input:IssueUpdateInput)=>Promise<void>;onDelete:()=>Promise<void>;onCreateSubIssue:(input:SubIssueInput)=>Promise<void>;onReactIssue:(emoji:string)=>Promise<void>;onComment:(body:string,bodyData?:Record<string,unknown>,parentId?:string)=>Promise<void>;onEditComment:(id:string,body:string,bodyData?:Record<string,unknown>)=>Promise<void>;onDeleteComment:(id:string)=>Promise<void>;onReactComment:(id:string,emoji:string)=>Promise<void>;onRelation:(type:IssueRelationType,relatedIssueId:string)=>Promise<void>;onDeleteRelation:(relationId:string)=>Promise<void>;onUpload:(file:File)=>Promise<void>;onDeleteAttachment:(attachmentId:string)=>Promise<void>}){
  ...
  return <section className={`issue-view ${full?'full':''} ${embedded?'issue-view--embedded':''}`}>
    {!embedded&&<IssueHeader
      ...
      onNavigateIssue={openIssue} onNavigateRoot={onNavigateRoot?()=>{void flush().then(saved=>{if(saved)onNavigateRoot()})}:undefined}
    ...
    <div className="issue-scroll"><div className="issue-layout">
      <article className="issue-document">
        ...
        <div className="issue-mobile-properties" aria-label="Issue properties">
          <StatusPicker value={issue.state} states={availableStates} onChange={stateId=>onUpdate({stateId})}/><PriorityPicker value={issue.priority} onChange={priority=>onUpdate({priority})}/><AssigneePicker value={issue.assignee} users={data.users} onChange={assigneeId=>onUpdate({assigneeId})}/><LabelPicker value={issue.labels} labels={availableLabels} labelGroups={data.labelGroups} onToggle={toggleLabel}/><ProjectPicker value={issue.project} projects={data.projects} teamName={issue.team.name} onChange={projectId=>onUpdate({projectId})}/>{releasesEnabled&&<IssueReleasePicker data={data} issue={issue}/>}
        </div>
        <IssueDescriptionEditor
          value={description}
          state={issue.documentContent?.contentData?JSON.stringify(issue.documentContent.contentData):issue.descriptionState}
          onBlur={()=>void flush()}
          onChange={snapshot=>setDescription(snapshot.markdown)}
          collaboration={{
            workspaceKey:data.workspace.urlKey,
            issueId:issue.id,
            documentId:issue.documentContent?.id??`document_content_${issue.id}`,
            contentState:issue.documentContent?.contentState,
            viewer:data.viewer,
            onPersist:(snapshot,updateIds)=>onUpdate({description:snapshot.markdown,descriptionState:snapshot.documentJSON,descriptionData:snapshot.document as Record<string,unknown>,contentState:snapshot.contentState,documentUpdateIds:updateIds,expectedDocumentVersion:issue.documentContent?.version??0}),
          }}
        />
      ...
      <IssueProperties issue={issue} data={data} activities={activities} related={related} releasesEnabled={releasesEnabled} onUpdate={onUpdate} onToggleLabel={toggleLabel}/>
    </div></div>
...
type SubIssueProperty='priority'|'id'|'status'|'labels'|'project'|'cycle'|'dueDate'|'links'|'assignee'
type SubIssueDisplay={ordering:'priority'|'created'|'updated';direction:'asc'|'desc';completed:'all'|'none';nested:boolean;properties:Set<SubIssueProperty>}
...
function DetailSubIssueRow({child,data,depth,properties,selected,onOpen,onSelect}:{child:Issue;data:BootstrapData;depth:number;properties:Set<SubIssueProperty>;selected:boolean;onOpen:()=>void;onSelect:()=>void}){const labels=child.labels.slice(0,3),cycle=data.cycles.find(item=>item.id===child.cycleId),hasStatus=properties.has('status');return <a className="detail-sub-issue-row" data-selected={selected} data-depth={depth} data-has-status={hasStatus} style={{'--sub-depth':depth} as React.CSSProperties} href={`/${data.workspace.urlKey}/issue/${child.identifier}`} onClick={event=>{if((event.target as Element).closest('[role=checkbox]')){event.preventDefault();return}event.preventDefault();onOpen()}}><span className="detail-sub-checkbox" role="checkbox" tabIndex={0} aria-label="Select issue" aria-checked={selected} onClick={event=>{event.preventDefault();event.stopPropagation();onSelect()}} onKeyDown={event=>{if(event.key===' '||event.key==='Enter'){event.preventDefault();onSelect()}}}>{selected&&<CheckboxMark/>}</span>{hasStatus&&<StatusIcon state={child.state} size={14}/>}<span className="detail-sub-title" data-i18n-ignore>{properties.has('id')&&<small>{child.identifier}</small>}{child.title}</span><span className="detail-sub-properties">{properties.has('priority')&&child.priority>0&&<span className="detail-sub-priority">{child.priorityLabel}</span>}{properties.has('labels')&&labels.map(label=><span className="detail-sub-badge" key={label.id}><i style={{background:label.color}}/><b data-i18n-ignore>{label.name}</b></span>)}{properties.has('project')&&child.project&&<span className="detail-sub-badge"><b data-i18n-ignore>{child.project.name}</b></span>}{properties.has('cycle')&&cycle&&<span className="detail-sub-badge"><b data-i18n-ignore>{cycle.name}</b></span>}{properties.has('dueDate')&&child.dueDate&&<span className="detail-sub-badge"><b>{child.dueDate.slice(5)}</b></span>}{properties.has('links')&&child.attachments.length>0&&<span className="detail-sub-badge"><Link2/><b>{child.attachments.length}</b></span>}{properties.has('assignee')&&child.assignee&&<Avatar name={child.assignee.displayName}/>}</span></a>}

function SubIssueDisplayMenu({value,onChange}:{value:SubIssueDisplay;onChange:(value:SubIssueDisplay)=>void}){const{t}=useI18n();const toggle=(property:SubIssueProperty)=>{const properties=new Set(value.properties);if(properties.has(property))properties.delete(property);else properties.add(property);onChange({...value,properties})};return <Popover.Root><Popover.Trigger asChild><button className="sub-issues-display" aria-label={t('Display options')}><DisplayIcon/></button></Popover.Trigger><Popover.Portal><Popover.Content className="sub-issues-display-menu" align="end" sideOffset={4}><label><span>{t('Ordering')}</span><span className="sub-issue-order"><select aria-label={t('Ordering')} value={value.ordering} onChange={event=>onChange({...value,ordering:event.target.value as SubIssueDisplay['ordering']})}><option value="priority">{t('Priority')}</option><option value="created">{t('Created')}</option><option value="updated">{t('Updated')}</option></select><button aria-label={t('Direction')} onClick={()=>onChange({...value,direction:value.direction==='asc'?'desc':'asc'})}>{value.direction==='asc'?'↑':'↓'}</button></span></label><label><span>{t('Completed issues')}</span><select aria-label={t('Completed issues')} value={value.completed} onChange={event=>onChange({...value,completed:event.target.value as SubIssueDisplay['completed']})}><option value="all">{t('All')}</option><option value="none">{t('None')}</option></select></label><label><span>{t('Nested sub-issues')}</span><Toggle checked={value.nested} label={t('Nested sub-issues')} onChange={nested=>onChange({...value,nested})}/></label><h4>{t('Display properties')}</h4><div>{(['priority','id','status','labels','project','cycle','dueDate','links','assignee'] as SubIssueProperty[]).map(property=><button aria-pressed={value.properties.has(property)} key={property} onClick={()=>toggle(property)}>{t(property==='dueDate'?'Due date':property==='id'?'ID':property[0].toUpperCase()+property.slice(1))}</button>)}</div></Popover.Content></Popover.Portal></Popover.Root>}

function IssueProperties({issue,data,activities,related,releasesEnabled,onUpdate,onToggleLabel}:{issue:Issue;data:BootstrapData;activities:ActivityEvent[];related:(id:string)=>Issue|undefined;releasesEnabled:boolean;onUpdate:(i:IssueUpdateInput)=>Promise<void>;onToggleLabel:(id:string)=>Promise<void>}){const labels=labelsForResource(data.labels,'issue').filter(label=>!label.scope||label.scope==='Workspace'||label.scope===issue.team.id);const sla=data.issueSlas.find(item=>item.issueId===issue.id&&item.status!=='removed');const rule=sla?data.slaRules.find(item=>item.id===sla.ruleId):undefined;const member=issue.assignee?data.members.find(item=>item.user.id===issue.assignee?.id):undefined;return <aside className="issue-properties"><h3>Properties</h3><StatusPicker value={issue.state} states={statesForIssue(data,issue)} hoverHistory={{activities,issueCreatedAt:issue.createdAt}} onChange={stateId=>onUpdate({stateId})}/><PriorityPicker value={issue.priority} onChange={priority=>onUpdate({priority})}/><div className="issue-assignee-property"><AssigneePicker value={issue.assignee} users={data.users} hoverContext={{member,workspaceName:issue.team.name,project:issue.project}} onChange={assigneeId=>onUpdate({assigneeId})}/>{issue.assignee&&<a className="issue-assignee-profile" href={`/${data.workspace.urlKey}/members`} aria-label="Go to user"><ChevronRight size={15}/></a>}</div>{sla&&<PropertyGroup title="SLA"><IssueSLAIndicator sla={sla} ruleName={rule?.name}/></PropertyGroup>}<PropertyGroup title="Labels"><LabelPicker inline value={issue.labels} labels={labels} labelGroups={data.labelGroups} onToggle={onToggleLabel}/></PropertyGroup><PropertyGroup title="Project"><ProjectPicker value={issue.project} projects={data.projects} teamName={issue.team.name} onChange={projectId=>onUpdate({projectId})}/></PropertyGroup>{issue.parentId&&<PropertyGroup title="Parent"><PropertyMenu compact label="Parent" value={`${related(issue.parentId)?.identifier} ${related(issue.parentId)?.title}`} options={[{id:'',label:'No parent'},...data.issues.filter(i=>i.id!==issue.id).map(i=>({id:i.id,label:`${i.identifier} ${i.title}`}))]} onChange={parentId=>onUpdate({parentId})}/></PropertyGroup>}{issue.dueDate&&<PropertyGroup title="Due date"><DueDatePicker value={issue.dueDate} onChange={dueDate=>onUpdate({dueDate})}/></PropertyGroup>}{releasesEnabled&&<PropertyGroup title="Releases"><IssueReleasePicker data={data} issue={issue}/></PropertyGroup>}</aside>}
