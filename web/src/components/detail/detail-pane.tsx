...
import type { ActivityEvent, Attachment, BootstrapData, CodeReview, Comment, FlowDocument, Issue, IssueRelationType, IssueUpdateInput, Presence, Project, ProjectMilestone, WorkflowState } from '@/types/flow'
...
export function DetailPane({issue,data,comments,activities,historyLoading=false,historyError,onRetryHistory,highlightTarget,presence=[],workspacePresence,full=false,embedded=false,issueOptionsActions,onClose,onNavigateIssue,onNavigateRoot,returnPath,navigationIssueIds,onUpdate,onIssueUpdated,onDelete,onCreateSubIssue,onCreateProject,onCreateProjectMilestone,onReactIssue,onComment,onEditComment,onDeleteComment,onReactComment,onRelation,onDeleteRelation,onUpload,onDeleteAttachment}:{issue:Issue;data:BootstrapData;comments:Comment[];activities:ActivityEvent[];historyLoading?:boolean;historyError?:string;onRetryHistory?:()=>void;highlightTarget?:ActivityHighlightTarget;presence?:Presence[];workspacePresence?:Presence[];full?:boolean;embedded?:boolean;issueOptionsActions?:IssueOptionsActions;onClose:()=>void;onExpand?:()=>void;onNavigateIssue?:(issue:Issue)=>void;onNavigateRoot?:()=>void;returnPath?:string;navigationIssueIds?:string[];onUpdate:(input:IssueUpdateInput)=>Promise<void>;onIssueUpdated?:(issue:Issue)=>void;onDelete:()=>Promise<void>;onCreateSubIssue:(input:SubIssueInput)=>Promise<void>;onCreateProject?:(draft:NewProjectDraft)=>Promise<Project>;onCreateProjectMilestone?:(projectId:string,input:{name:string})=>Promise<ProjectMilestone>;onReactIssue:(emoji:string)=>Promise<void>;onComment:(body:string,bodyData?:Record<string,unknown>,parentId?:string)=>Promise<void>;onEditComment:(id:string,body:string,bodyData?:Record<string,unknown>)=>Promise<void>;onDeleteComment:(id:string)=>Promise<void>;onReactComment:(id:string,emoji:string)=>Promise<void>;onRelation:(type:IssueRelationType,relatedIssueId:string)=>Promise<void>;onDeleteRelation:(relationId:string)=>Promise<void>;onUpload:(file:File, options?:{embed?:boolean})=>Promise<Attachment|void>;onDeleteAttachment:(attachmentId:string)=>Promise<void>}){
  ...
  const descriptionSelection = useDescriptionSelectionActions(issue, data, onComment)
      ...
      <article className="issue-document">
        ...
        <IssueDescriptionEditor
          ...
          collaboration={{
            ...
            documentVersion:issue.documentContent?.version??0,
            viewer:data.viewer,
            onPersist:(snapshot,updateIds,expectedVersion)=>onUpdate({description:snapshot.markdown,descriptionState:snapshot.documentJSON,descriptionData:snapshot.document as Record<string,unknown>,contentState:snapshot.contentState,documentUpdateIds:updateIds,expectedDocumentVersion:expectedVersion}),
          }}
        />
...
function IssueProperties({issue,data,activities,presence=[],releasesEnabled,onCreateMilestone,onCreateProject,onUpdate,onToggleLabel}:{issue:Issue;data:BootstrapData;activities:ActivityEvent[];presence?:Presence[];releasesEnabled:boolean;onCreateMilestone?:(projectId:string,input:{name:string})=>Promise<ProjectMilestone>;onCreateProject?:(draft:NewProjectDraft)=>Promise<Project>;onUpdate:(i:IssueUpdateInput)=>Promise<void>;onToggleLabel:(id:string)=>Promise<void>}){const labelScopes=new Set(labelTeamScopeIds(issue.team.id,data.teams,data.teamSettings));const labels=labelsForResource(data.labels,'issue',data.labelGroups).filter(label=>!label.scope||label.scope==='Workspace'||labelScopes.has(label.scope));const sla=data.issueSlas.find(item=>item.issueId===issue.id&&item.status!=='removed');const rule=sla?data.slaRules.find(item=>item.id===sla.ruleId):undefined;const member=issue.assignee?data.members.find(item=>item.user.id===issue.assignee?.id):undefined;const online=Boolean(issue.assignee&&(issue.assignee.id===data.viewer.id||presence.some(item=>item.user.id===issue.assignee?.id)));const estimateType=resolvedTeamSettings(data.teamSettings,issue.team.id)?.estimateType??'notUsed';return <aside className="issue-properties"><h3>Properties</h3><StatusPicker value={issue.state} states={statesForIssue(data,issue)} hoverHistory={{activities,issueCreatedAt:issue.createdAt}} onChange={stateId=>onUpdate({stateId})}/><PriorityPicker value={issue.priority} onChange={priority=>onUpdate({priority})}/>{estimateType!=='notUsed'&&<EstimatePicker value={issue.estimate} estimateType={estimateType} onChange={estimate=>onUpdate({estimate})}/>}<div className="issue-assignee-property"><AssigneePicker value={issue.assignee} users={data.users} hoverContext={{member,online,workspaceName:issue.team.name,project:issue.project}} onChange={assigneeId=>onUpdate({assigneeId})}/>{issue.assignee&&<a className="issue-assignee-profile" href={`/${data.workspace.urlKey}/members`} aria-label="Go to user"><ChevronRight size={15}/></a>}</div><IssueAgentPicker issue={issue} data={data} onUpdate={onUpdate}/>{data.cycleSettings[issue.team.id]?.enabled === true && <CyclePicker valueId={issue.cycleId} cycles={data.cycles} issues={data.issues} teamId={issue.team.id} onChange={cycleId=>onUpdate({cycleId})}/>}{sla&&<PropertyGroup title="SLA"><IssueSLAIndicator sla={sla} ruleName={rule?.name}/></PropertyGroup>}<PropertyGroup title="Labels"><LabelPicker inline value={issue.labels} labels={labels} labelGroups={data.labelGroups} onToggle={onToggleLabel}/></PropertyGroup><IssueProjectPicker grouped data={data} issue={issue} presence={presence} onCreateMilestone={onCreateMilestone} onCreateProject={onCreateProject} onUpdate={onUpdate}/>{issue.dueDate&&<PropertyGroup title="Due date"><DueDatePicker value={issue.dueDate} onChange={dueDate=>onUpdate({dueDate})}/></PropertyGroup>}{releasesEnabled&&<IssueReleasePicker grouped data={data} issue={issue}/>}</aside>}
...
function IssueResources({documents,links,workspaceKey,actions,onDeleteLink}:{documents:FlowDocument[];links:Attachment[];workspaceKey:string;actions?:IssueOptionsActions;onDeleteLink:(id:string)=>Promise<void>}){
  const[collapsed,setCollapsed]=useState(false),[menuOpen,setMenuOpen]=useState(false),[linkOpen,setLinkOpen]=useState(false),[url,setUrl]=useState(''),[title,setTitle]=useState(''),[saving,setSaving]=useState(false)
  const addLink=async()=>{if(!actions||!url.trim())return;setSaving(true);try{await actions.addLink({url:url.trim(),title:title.trim()||undefined});setUrl('');setTitle('');setLinkOpen(false)}finally{setSaving(false)}}
  return <section className="issue-detail-section issue-resources"><header><button className="issue-section-collapse" aria-label={collapsed?'Expand resources section':'Collapse resources section'} aria-expanded={!collapsed} onClick={()=>setCollapsed(value=>!value)}><ChevronDown/><span>Resources</span></button><span>{documents.length+links.length}</span><span/><Popover.Root open={menuOpen} onOpenChange={setMenuOpen}><Popover.Trigger asChild><button className="issue-section-add" aria-label="Add resource to issue"><Plus/></button></Popover.Trigger><Popover.Portal><Popover.Content data-flow-motion="floating" className="issue-resource-menu" align="end" sideOffset={4}><button onClick={()=>{setMenuOpen(false);void actions?.addDocument()}} disabled={!actions}><FileText/>Create document</button><button onClick={()=>{setMenuOpen(false);setLinkOpen(true)}} disabled={!actions}><Link2/>Add link</button></Popover.Content></Popover.Portal></Popover.Root></header>{!collapsed&&<>{documents.map(document=><a className="linked-issue issue-resource-row" href={documentPath(workspaceKey,document)} key={document.id}><DocumentGlyph document={document}/><span>Document</span><strong data-i18n-ignore>{document.title}</strong></a>)}{links.map(link=><a className="linked-issue issue-resource-row" href={link.url} target="_blank" rel="noreferrer" key={link.id}><Link2 size={14}/><span>{safeHost(link.url)}</span><strong data-i18n-ignore>{link.title}</strong><button aria-label={`Delete resource ${link.title}`} onClick={event=>{event.preventDefault();event.stopPropagation();void onDeleteLink(link.id)}}><X/></button></a>)}</>}
    <Dialog.Root open={linkOpen} onOpenChange={setLinkOpen}><Dialog.Portal><Dialog.Overlay data-flow-motion="backdrop" className="modal-overlay"/><Dialog.Content data-flow-motion="dialog" className="issue-link-dialog"><Dialog.Title>Add link</Dialog.Title><label>URL<input autoFocus type="url" value={url} placeholder="https://" onChange={event=>setUrl(event.target.value)}/></label><label>Title<input value={title} placeholder="Optional" onChange={event=>setTitle(event.target.value)} onKeyDown={event=>{if(event.key==='Enter'&&url.trim())void addLink()}}/></label><footer><Dialog.Close asChild><button>Cancel</button></Dialog.Close><button className="primary" disabled={!url.trim()||saving} onClick={()=>void addLink()}>{saving?'Adding…':'Add link'}</button></footer></Dialog.Content></Dialog.Portal></Dialog.Root>
  </section>
}
