...
import { ArrowDown, ArrowUp, Bell, BellOff, Building2, Check, ChevronRight, Copy, Ellipsis, Layers3, LockKeyhole, Pencil, Star, Trash2, UserRound, Users } from 'lucide-react'
...
export function ViewsPage({ data, resource, scope, views, onCreate, onDelete, onDuplicate, onOpen, onOpenSidebar, onResourceChange, resourceHref, onUpdate, viewHref }: ViewsPageProps) {
  ...
  return <div className={styles.page}>
    ...
    <div className={styles.toolbar}>
      ...
      <DisplayMenu direction={direction} ordering={ordering} properties={properties} onDirection={changeDirection} onOrdering={next => { setOrdering(next); writePreference(`${storageKey}:ordering`, next) }} onToggleProperty={toggleProperty}/>
    </div>
...
function DisplayMenu({ direction, ordering, properties, onDirection, onOrdering, onToggleProperty }: { direction: Direction; ordering: Ordering; properties: Set<DisplayProperty>; onDirection: () => void; onOrdering: (ordering: Ordering) => void; onToggleProperty: (property: DisplayProperty) => void }) {
  return <Popover.Root>
...
function ScopeAvatar({ kind, label }: { kind: SavedView['scope']; label: string }) { return <span className={styles.scopeAvatar}>{kind === 'personal' ? <UserRound size={13}/> : kind === 'team' ? <Users size={13}/> : label.slice(0, 2).toUpperCase()}</span> }
