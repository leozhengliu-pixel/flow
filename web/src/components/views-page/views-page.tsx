...
export function ViewsPage({ data, resource, scope, views, onCreate, onDelete, onDuplicate, onEdit, onOpen, onOpenSidebar, onResourceChange, resourceHref, onUpdate, onToggleFavorite, onSetSubscriptionEvents, onShare, viewHref }: ViewsPageProps) {
  ...
  const deleteView = async (view: SavedView) => {
    if (!await confirmAction(`Delete view “${view.name}”?`,{description:'This cannot be undone.',confirmLabel:'Delete view'})) return
    await onDelete(view)
  }
...
function SortButton({ active, direction, label, onClick }: { active: boolean; direction: Direction; label: string; onClick: () => void }) { return <button aria-label={`Order by ${label}`} className={styles.sortButton} data-active={active || undefined} onClick={onClick} type="button"><span>{label}</span>{direction === 'asc' ? <ArrowDown aria-hidden="true" size={12}/> : <ArrowUp aria-hidden="true" size={12}/>}</button> }
