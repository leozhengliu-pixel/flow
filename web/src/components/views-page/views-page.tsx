...
export function ViewsPage({
  ...
  const [ordering, setOrdering] = useState<Ordering>(
    () =>
      readPreference(
        `${storageKey}:ordering`,
        scope.kind === "team" ? "owner" : "name",
      ) as Ordering,
  );
  const [direction, setDirection] = useState<Direction>(
    () => readPreference(`${storageKey}:direction`, "asc") as Direction,
  );
  ...
  const orderedViews = useMemo(
    () =>
      [...views].sort((left, right) =>
        compareViews(left, right, ordering, direction, usersById, data.viewer),
      ),
  ...
  const updateOrdering = (next: Ordering) => {
    const nextDirection =
      ordering === next ? (direction === "asc" ? "desc" : "asc") : "asc";
    setOrdering(next);
    setDirection(nextDirection);
    writePreference(`${storageKey}:ordering`, next);
    writePreference(`${storageKey}:direction`, nextDirection);
  };
  const changeDirection = () => {
    const next = direction === "asc" ? "desc" : "asc";
    setDirection(next);
    writePreference(`${storageKey}:direction`, next);
  };
  ...
  const moveView = (view: SavedView, destination: SavedViewMutationInput) =>
    onUpdate(view.id, {
      ...destination,
      teamId: destination.scope === "team" ? destination.teamId : "",
    });
  const copyLink = async (view: SavedView) => {
    await navigator.clipboard.writeText(
      `${window.location.origin}${viewHref(view)}`,
    );
    toast.success(t("View link copied"));
  };
  const deleteView = async (view: SavedView) => {
    if (
      !(await confirmAction(`${t("Delete view")} “${view.name}”?`, {
    ...
    <div className={styles.page}>
      <ViewsDirectoryHeader
        ...
        afterTitle={
          scope.kind === "team" && (
            <button
              aria-checked={teamFavorite}
              aria-label={t(
                teamFavorite ? "Remove from favorites" : "Add to favorites",
              )}
              className={styles.favoriteHeader}
              disabled={!onToggleScopeFavorite}
              onClick={() =>
                void onToggleScopeFavorite?.(scope.team.id, !teamFavorite)
              }
              role="switch"
              type="button"
            >
              <ViewFavoriteIcon selected={teamFavorite} />
            </button>
          )
        }
        onAction={onCreate}
        ...
        tabs={[
          {
            resource: "issues",
            label: "Issues",
            href: resourceHref("issues"),
            onSelect: () => onResourceChange("issues"),
          },
          {
            resource: "projects",
            label: "Projects",
...
function ViewRow({
  ...
  onDuplicate: () => void;
  onEdit: () => void;
  onMove: (input: SavedViewMutationInput) => void;
  onOpen: () => void;
  onSetSubscriptionEvents: (events: string[]) => void;
  onShare?: () => Promise<void>;
  onToggleFavorite: () => void;
  onUpdate: (input: SavedViewMutationInput) => void;
    ...
    >
      <div className={styles.identity}>
        ...
        <span data-i18n-ignore>
          ...
          <small>{viewDescription(view, resource, t)}</small>
        </span>
        {favorite && (
          <Star
            aria-label={t("Favorite")}
            className={styles.rowFavorite}
            fill="currentColor"
            size={11}
          />
        )}
    ...
    <ContextMenu.Root>
      ...
      <ContextMenu.Portal>
        ...
        >
          ...
          />
          <ContextMenu.Sub>
            <ContextMenu.SubTrigger className={styles.menuItem}>
              <UserRound />
              <span>{t("Owner")}</span>
            ...
            <ContextMenu.Portal>
              ...
              >
                <ViewMenuItem
                  icon={<LockKeyhole />}
                  label={t("Personal")}
                  onSelect={() => onMove({ scope: "personal" })}
                />
                <ViewMenuItem
                  icon={<Building2 />}
                  label={t("Workspace")}
                  onSelect={() => onMove({ scope: "workspace" })}
                />
            ...
            <ContextMenu.Portal>
              ...
              >
                <SubscriptionEventItem
                  checked={subscriptionEvents.has("issue-added")}
                  label={t(
                    `${resource === "issues" ? "An issue" : "A project"} is added to the view`,
                  )}
                  onSelect={() => toggleSubscriptionEvent("issue-added")}
                />
                <SubscriptionEventItem
                  checked={subscriptionEvents.has("issue-completed")}
                  label={t(
                    `${resource === "issues" ? "An issue is" : "A project is"} marked completed or canceled`,
                  )}
                  onSelect={() => toggleSubscriptionEvent("issue-completed")}
                />
              </ContextMenu.SubContent>
            </ContextMenu.Portal>
          </ContextMenu.Sub>
          ...
          {onShare && (
            <ViewMenuItem
              icon={view.shareToken ? <LockKeyhole /> : <Copy />}
              label={t(view.shareToken ? "Disable public link" : "Share view")}
              onSelect={() => {
                void onShare();
              }}
            />
...
function ViewsDisplayMenu({
  ...
  properties: Set<DisplayProperty>;
  onDirection: () => void;
  onOrdering: (ordering: Ordering) => void;
    ...
    <Popover.Root>
      <Popover.Trigger asChild>
        <button
          ...
          className={`${styles.displayTrigger} ui-pill`}
          type="button"
        >
          <DisplayOptionsIcon />
        </button>
        ...
        >
          <div className={styles.displayOrderingSection}>
            <div className={styles.displayOrderingRow}>
              <span className={styles.displayLabel}>{t("Ordering")}</span>
              <div className={styles.displayOrderingControls}>
                <button
                  ...
                  type="button"
                >
                  <OrderingDirectionIcon direction={direction} />
                </button>
                <Select.Root
                  onValueChange={(value) => onOrdering(value as Ordering)}
                  value={ordering}
                >
                  <Select.Trigger
                    aria-label={t("View ordering")}
                  ...
                  <Select.Portal>
                    ...
                    >
                      <Select.Viewport>
                        {(["name", "owner", "updated", "created"] as const).map(
                          (item) => (
                            <Select.Item
                            ...
                            >
                              ...
                              <Select.ItemIndicator
                                className={styles.orderingIndicator}
                              >
                                <FlowCheckIcon />
                              </Select.ItemIndicator>
                            </Select.Item>
          ...
          <div className={styles.displayPropertiesSection}>
            <span className={styles.displayPropertiesLabel}>
              {t("Display properties")}
            </span>
            <div className={styles.displayPropertyOptions}>
              {(["created", "updated", "owner"] as const).map((property) => (
                <button
                  aria-pressed={properties.has(property)}
                  className={styles.displayProperty}
                  data-active={properties.has(property) || undefined}
                  key={property}
                  onClick={() => onToggleProperty(property)}
                  type="button"
                >
                  {t(orderingLabel(property))}
                </button>
              ))}
            </div>
          </div>
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button
          ...
          type="button"
        >
          <ViewOwnerAvatar user={owner} />
          <span data-i18n-ignore>{owner.displayName}</span>
        </button>
...
function SubscriptionEventItem({
  checked,
  label,
  onSelect,
}: {
  ...
  return (
    ...
    >
      <span className={styles.menuCheckbox}>{checked && <CheckboxMark />}</span>
      <span>{label}</span>
    </ContextMenu.CheckboxItem>
  );
}
...
function ViewOwnerAvatar({ user }: { user: User }) {
  return (
    <UserAvatar
...
function ViewFavoriteIcon({ selected }: { selected: boolean }) {
  return selected ? (
    <svg aria-hidden="true" fill="lch(80% 90 85)" viewBox="0 0 16 16">
    ...
    <svg aria-hidden="true" fill="lch(61.803% 1.2 272)" viewBox="0 0 16 16">
...
function OrderingDirectionIcon({ direction }: { direction: Direction }) {
  return direction === "asc" ? (
    <svg aria-hidden="true" fill="currentColor" viewBox="0 0 16 16">
      <path
        clipRule="evenodd"
        d="M8.23 12.326a.75.75 0 1 0-.96-1.152L5.5 12.649V1.75a.75.75 0 0 0-1.5 0v10.899l-1.77-1.475a.75.75 0 1 0-.96 1.152l3 2.5a.75.75 0 0 0 .96 0z"
        fillRule="evenodd"
      />
      <path
        clipRule="evenodd"
        d="M7 8.75c0 .414.336.75.75.75h6.5a.75.75 0 0 0 0-1.5h-6.5a.75.75 0 0 0-.75.75M7 5.75c0 .414.336.75.75.75h4.5a.75.75 0 0 0 0-1.5h-4.5a.75.75 0 0 0-.75.75M7 2.75c0 .414.336.75.75.75h1.5a.75.75 0 1 0 0-1.5h-1.5a.75.75 0 0 0-.75.75"
        fillRule="evenodd"
      />
    </svg>
  ) : (
    <svg aria-hidden="true" fill="currentColor" viewBox="0 0 16 16">
      <path
        clipRule="evenodd"
        d="M1.27 3.674a.75.75 0 1 0 .96 1.152L4 3.351V14.25a.75.75 0 0 0 1.5 0V3.351l1.77 1.475a.75.75 0 1 0 .96-1.152l-3-2.5a.75.75 0 0 0-.96 0z"
        fillRule="evenodd"
      />
      <path
        clipRule="evenodd"
        d="M7 7.25A.75.75 0 0 1 7.75 6.5h6.5a.75.75 0 0 1 0 1.5h-6.5A.75.75 0 0 1 7 7.25M7 10.25a.75.75 0 0 1 .75-.75h4.5a.75.75 0 0 1 0 1.5h-4.5A.75.75 0 0 1 7 10.25M7 13.25a.75.75 0 0 1 .75-.75h1.5a.75.75 0 1 1 0 1.5h-1.5A.75.75 0 0 1 7 13.25"
        fillRule="evenodd"
      />
    </svg>
  );
}
function CompactChevron() {
  return (
    <svg aria-hidden="true" fill="currentColor" viewBox="0 0 9 5">
...
function orderingLabel(value: Ordering | DisplayProperty) {
...
function viewDescription(
  ...
  return count
    ...
    : translate(`All ${resource}`);
}
function columnTemplate(properties: Set<DisplayProperty>) {
  return `minmax(240px, 1fr)${properties.has("created") ? " 110px" : ""}${properties.has("updated") ? " 110px" : ""}${properties.has("owner") ? " 120px" : ""}`;
}
function readProperties(key: string): DisplayProperty[] {
  try {
    const value = JSON.parse(localStorage.getItem(key) ?? '["owner"]');
    return Array.isArray(value)
      ? value.filter((item) => ["created", "owner", "updated"].includes(item))
      : ["owner"];
  } catch {
    return ["owner"];
  }
...
function compareViews(
  ...
  const leftValue =
    ordering === "name"
      ? left.name
      : ordering === "owner"
        ? (users.get(left.ownerId ?? "") ?? viewer).displayName
        : ordering === "created"
          ? left.createdAt
          : left.updatedAt;
  const rightValue =
    ordering === "name"
      ? right.name
      : ordering === "owner"
        ? (users.get(right.ownerId ?? "") ?? viewer).displayName
        : ordering === "created"
          ? right.createdAt
          : right.updatedAt;
